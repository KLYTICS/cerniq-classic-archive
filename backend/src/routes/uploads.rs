...
pub async fn upload_file(
    ...
    while let Some(field) = multipart.next_field().await.map_err(|e| AppError::InvalidInput(e.to_string()))? {
        ...
        if name == "workspace_id" {
            ...
            workspace_id = Some(Uuid::parse_str(&text).map_err(|e| AppError::InvalidInput(format!("Invalid workspace_id: {}", e)))?);
        } else if name == "file" || field.file_name().is_some() {
            // This is the file field
            let file_name = field.file_name().unwrap_or("unknown").to_string();
            let data = field.bytes().await.map_err(|e| AppError::InvalidInput(e.to_string()))?;
            file_data = Some((file_name, data.to_vec()));
        }
    }

    // Validate we have both workspace_id and file
    let workspace_id = workspace_id.ok_or_else(|| AppError::InvalidInput("workspace_id is required".to_string()))?;
    let (file_name, data) = file_data.ok_or_else(|| AppError::InvalidInput("No file provided".to_string()))?;
    ...
    if file_type == "unknown" {
        return Err(AppError::InvalidInput("Unsupported file type. Please upload CSV or PDF files.".to_string()));
    }
    ...
    let mut file = fs::File::create(&file_path).await.map_err(|e| AppError::Internal(e.to_string()))?;
    file.write_all(&data).await.map_err(|e| AppError::Internal(e.to_string()))?;
