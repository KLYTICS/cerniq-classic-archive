...
| ✅ Body-tenancy IDOR enforcement (R3) | n/a | `verify-body-trust.mjs --strict` wired into `npm run lint` immediately after `verify:auth-coverage` (`<pending>`). Self-test 9/9 green. Catches the body-IDOR class (`parseOrThrow(<Schema>, body|query)` where `<Schema>` declares `{institutionId, organizationId, orgId, clientInstitutionId, workspaceId}` without a paired `verify*Ownership`/`verify*Membership`/`assert*Access` call OR `// verify:body-trust-skip — <reason>` annotation). First catch in `19f103f5`: `ncua.controller.ts:55` `importCreditUnion` body-IDOR — closed in `58651c54` via new `InstitutionScopeGuard.verifyWorkspaceOwnership` primitive. Live state: 60 controllers, 510 handlers, 27 tenancy-bearing schemas, 0 violations. |
