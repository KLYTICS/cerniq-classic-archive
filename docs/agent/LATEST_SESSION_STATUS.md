...
- Integration note (2026-04-11): merged `codex/export-local-green` parallel work (ALM document exports, density components, institution intelligence, command palette rewrite). Baseline numbers from that branch: public gate `12/12`, Vitest `475/475`, backend unit `5630/5630`, e2e/security `64/64`. Treat the HEAD acceptance line above as the post-merge target; re-run the gates to reconfirm before relying on the combined numbers.
