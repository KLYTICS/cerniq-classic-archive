...
const DEMO_CLIENTS: CPAClient[] = [
  { id: 'c1', institutionName: 'Cooperativa de Ahorro Caguas', totalAssets: 2_800_000_000, riskScore: 85, lastAnalysisDate: '2026-04-14T00:00:00Z', complianceStatus: 'compliant' },
  { id: 'c2', institutionName: 'ACACIA Federal Credit Union', totalAssets: 1_500_000_000, riskScore: 72, lastAnalysisDate: '2026-04-13T00:00:00Z', complianceStatus: 'warning' },
  { id: 'c3', institutionName: 'Oriental Federal Credit Union', totalAssets: 1_200_000_000, riskScore: 91, lastAnalysisDate: '2026-04-15T00:00:00Z', complianceStatus: 'compliant' },
  { id: 'c4', institutionName: 'Cooperativa de Bayamon', totalAssets: 950_000_000, riskScore: 55, lastAnalysisDate: '2026-04-10T00:00:00Z', complianceStatus: 'breach' },
  { id: 'c5', institutionName: 'Cooperativa Jesus Obrero', totalAssets: 680_000_000, riskScore: 78, lastAnalysisDate: '2026-04-12T00:00:00Z', complianceStatus: 'compliant' },
  { id: 'c6', institutionName: 'Cooperativa de Arecibo', totalAssets: 520_000_000, riskScore: 63, lastAnalysisDate: '2026-04-11T00:00:00Z', complianceStatus: 'warning' },
];
...
function ComplianceBadge({ status, locale }: { status: string; locale: 'en' | 'es' }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold ${styles[status] || styles.warning}`}>
...
export default function CPADashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const firmId = typeof window !== 'undefined' ? sessionStorage.getItem('cpa_firm_id') || 'demo' : 'demo';
        const [dashRes, clientsRes] = await Promise.all([
          fetch(`${API}/api/cpa/firms/${firmId}/dashboard`, { headers: authHeaders() }),
          fetch(`${API}/api/cpa/firms/${firmId}/clients`, { headers: authHeaders() }),
        ]);
        if (dashRes.ok) {
  ...
  const medRisk = clients.filter((c) => c.riskScore >= 60 && c.riskScore < 80).length;
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {firm.totalClients} {locale === 'es' ? 'clientes' : 'clients'} &middot; {currencyCompact(firm.totalAUM)} AUM
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6 lg:col-span-3">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="rounded-xl border border-rose-200 bg-rose-50 p-4">
                ...
                <p className="mt-1 text-3xl font-bold text-rose-700">{highRisk}</p>
                <p className="mt-0.5 text-[11px] text-rose-500">{locale === 'es' ? 'Puntuacion < 60' : 'Score < 60'}</p>
              </div>
              <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-amber-600">
                  {locale === 'es' ? 'Riesgo Moderado' : 'Medium Risk'}
                </p>
                <p className="mt-1 text-3xl font-bold text-amber-700">{medRisk}</p>
                <p className="mt-0.5 text-[11px] text-amber-500">{locale === 'es' ? 'Puntuacion 60-79' : 'Score 60-79'}</p>
              </div>
              <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-emerald-600">
                  {locale === 'es' ? 'Bajo Riesgo' : 'Low Risk'}
                </p>
                <p className="mt-1 text-3xl font-bold text-emerald-700">{lowRisk}</p>
                <p className="mt-0.5 text-[11px] text-emerald-500">{locale === 'es' ? 'Puntuacion 80+' : 'Score 80+'}</p>
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {clients.map((client) => (
                      <tr key={client.id} className="border-b border-slate-50 transition hover:bg-slate-50">
                        <td className="px-5 py-3">
                          <span className="text-sm font-medium text-slate-800">{client.institutionName}</span>
                        </td>
                        <td className="px-5 py-3 text-right">
                          <span className="font-mono text-sm tabular-nums text-slate-700">{currencyCompact(client.totalAssets)}</span>
                        </td>
                        <td className="px-5 py-3 text-center">
                          <RiskScoreBadge score={client.riskScore} size="sm" />
                        </td>
                        <td className="px-5 py-3 text-center">
                          <span className="text-xs text-slate-500">{formatDate(client.lastAnalysisDate)}</span>
                        </td>
                        <td className="px-5 py-3 text-center">
                          <ComplianceBadge status={client.complianceStatus} locale={locale} />
                        </td>
                        <td className="px-5 py-3 text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button
                              className="rounded-lg border border-[#1e3a5f] px-2.5 py-1 text-[11px] font-medium text-[#1e3a5f] transition hover:bg-blue-50"
                            >
          ...
          <aside className="space-y-4">
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">
                {locale === 'es' ? 'Activos Bajo Asesoria' : 'Assets Under Advisory'}
              </p>
              <p className="mt-1 text-2xl font-bold text-[#1e3a5f]">{currencyCompact(metrics.totalAssetsUnderAdvisory)}</p>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <p className="mt-1 text-2xl font-bold text-amber-600">{metrics.upcomingExams}</p>
              <p className="mt-0.5 text-[11px] text-slate-400">{locale === 'es' ? 'Proximos 90 dias' : 'Next 90 days'}</p>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <p className="mt-1 text-2xl font-bold text-rose-600">{metrics.recentAlerts}</p>
              <p className="mt-0.5 text-[11px] text-slate-400">{locale === 'es' ? 'Ultimos 7 dias' : 'Last 7 days'}</p>
            </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              ...
              <div className="mt-3 space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-semibold text-slate-700">{highRisk}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-xs text-slate-600">
                  ...
                  <span className="text-xs font-semibold text-slate-700">{medRisk}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="flex items-center gap-1.5 text-xs text-slate-600">
                  ...
                  <span className="text-xs font-semibold text-slate-700">{lowRisk}</span>
                </div>
              </div>
            </div>
          </aside>
      ...
      {/* Add Client Modal */}
      <Modal open={addClientOpen} onClose={() => setAddClientOpen(false)} title={locale === 'es' ? 'Agregar Cliente' : 'Add Client'} maxWidth="max-w-md">
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder={locale === 'es' ? 'Nombre de cooperativa...' : 'Cooperativa name...'}
          ...
          {searchQuery.length > 2 && (
            <div className="space-y-1 rounded-lg border border-slate-200 p-2">
              {['Cooperativa del Interior', 'Cooperativa Roosevelt Roads', 'Cooperativa San Rafael']
                ...
                .map((name) => (
                  <button
                    ...
                    onClick={() => { setAddClientOpen(false); setSearchQuery(''); }}
                  >
                    {name}
                  </button>
                ))}
      ...
      {/* Bulk Upload Modal */}
      <Modal open={bulkUploadOpen} onClose={() => setBulkUploadOpen(false)} title={locale === 'es' ? 'Carga Masiva de Clientes' : 'Bulk Client Upload'} maxWidth="max-w-md">
        <div className="space-y-4">
          <div
            ...
            className={`flex flex-col items-center justify-center rounded-xl border-2 border-dashed p-8 transition ${
              dragOver ? 'border-[#1e3a5f] bg-blue-50' : 'border-slate-300 bg-slate-50'
            }`}
          >
            ...
            <p className="mt-1 text-xs text-slate-400">
              {locale === 'es' ? 'o haga clic para seleccionar' : 'or click to browse'}
            </p>
            <input type="file" accept=".csv" className="absolute inset-0 cursor-pointer opacity-0" aria-label="Upload CSV" />
          </div>
