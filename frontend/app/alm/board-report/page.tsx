  ...
  return (
    <div className="p-6 space-y-5 max-w-[1400px] mx-auto">
        ...
        {data.sections.map((s: any, i: number) => (
          <div key={i} className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-semibold text-slate-800">{locale === 'es' ? s.titleEs : s.title}</p>
              <span className="text-[10px] text-slate-400">p.{s.pageRange}</span>
            </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        {data.regPulse.map((r: any, i: number) => (
          <div key={i} className="flex items-center gap-3 py-1.5 border-b border-slate-50 last:border-0">
            <Calendar className="h-3.5 w-3.5 text-slate-400" />
            <span className="text-xs text-slate-700 flex-1">{locale === 'es' ? r.deadlineEs : r.deadline}</span>
            <span className="text-[10px] tabular-nums text-slate-500">{r.date}</span>
            <span className={`text-[10px] font-bold ${r.urgency === 'HIGH' ? 'text-rose-600' : 'text-amber-600'}`}>{r.urgency}</span>
          </div>
        ))}
      </div>
    </div>
