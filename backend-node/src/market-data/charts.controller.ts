...
export class ChartsController {
            ...
            return await this.cacheService.getOrSet(
                ...
                async () => {
                    ...
                    if (requestedIndicators.includes('bollinger')) {
                        indicators.bollingerBands = this.technicalService.calculateBollingerBands(closes);
                    }
                    if (requestedIndicators.includes('vwap')) {
                        indicators.vwap = this.technicalService.calculateVWAP(closes, volumes);
                    }
                    if (requestedIndicators.includes('atr')) {
                        indicators.atr = this.technicalService.calculateATR(highs, lows, closes);
                    }
     ...
     */
    @Get('ohlcv/:ticker')
    async getOHLCVData(@Param('ticker') ticker: string, @Query('timeframe') timeframe: string = '1M') {
        try {
