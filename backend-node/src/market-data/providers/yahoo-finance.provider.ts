...
export class YahooFinanceProvider {
  ...
  async getQuote(ticker: string): Promise<QuoteDto | null> {
    try {
      ...
      return {
        ...
        shortName:
          quote.shortName || quote.displayName || quote.symbol || ticker,
        longName:
          quote.longName ||
          quote.shortName ||
          quote.displayName ||
  ...
  async getHistoricalPrices(
    ...
    try {
      this.logger.debug(
        `Fetching historical prices for ${ticker} from ${startDate} to ${endDate}`,
      );
  ...
  async getInstrumentProfile(
    ticker: string,
  ): Promise<InstrumentProfileDto | null> {
    try {
      ...
      return {
        ticker:
          price.symbol || summary.quoteType?.symbol || ticker.toUpperCase(),
        assetType,
        shortName:
          summary.quoteType?.shortName || price.shortName || price.displayName,
        longName:
        ...
        categoryName:
          ...
          defaultKeyStatistics.category ||
          undefined,
        family:
          fundProfile.family || defaultKeyStatistics.fundFamily || undefined,
        description:
      ...
      this.logger.error(
        `Failed to fetch instrument profile for ${ticker}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  async searchTickers(query: string): Promise<TickerSearchResultDto[]> {
    try {
      ...
      this.logger.error(
        `Failed to search tickers with query ${query}: ${error.message}`,
      );
      return [];
    }
  }
