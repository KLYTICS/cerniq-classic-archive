...
export class CoinGeckoProvider {
    ...
    async getQuote(ticker: string): Promise<QuoteDto | null> {
        try {
            ...
            this.logger.error(`Failed to fetch crypto quote for ${ticker}: ${error.message}`);
            return null;
        }
    }
    ...
    async getHistoricalPrices(
        ...
        try {
            ...
            this.logger.debug(`Fetching ${days} days of historical data for ${ticker}`);
    ...
    async searchCrypto(query: string): Promise<Array<{ symbol: string; name: string; id: string }>> {
        try {
            ...
            this.logger.error(`Failed to search crypto with query ${query}: ${error.message}`);
            return [];
        }
    }
}
