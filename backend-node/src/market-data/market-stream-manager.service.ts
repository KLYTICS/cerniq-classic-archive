...
import { InstrumentProfileDto, NewsArticleDto, QuoteDto, StreamStatusDto } from './dto/quote.dto';
...
export class MarketStreamManagerService implements OnModuleDestroy {
    ...
    private readonly quotePollIntervalMs = this.parseInterval(process.env.MARKET_STREAM_INTERVAL_MS, 5000);
    private readonly profilePollIntervalMs = this.parseInterval(process.env.MARKET_PROFILE_STREAM_INTERVAL_MS, 15 * 60 * 1000);
    private readonly newsPollIntervalMs = this.parseInterval(process.env.MARKET_NEWS_STREAM_INTERVAL_MS, 5 * 60 * 1000);

    constructor(private readonly marketDataService: MarketDataService) {
        this.emitter.setMaxListeners(0);
    }

    private parseInterval(rawValue: string | undefined, fallbackMs: number): number {
    ...
    async subscribe(rawTicker: string): Promise<{ ticker: string; subscribers: number }> {
    ...
    private async publishQuote(ticker: string) {
        ...
        try {
            ...
            state.lastErrorMessage = error?.message || `Failed to stream quote for ${ticker}`;
    ...
    private async publishProfile(ticker: string) {
        ...
        try {
            ...
            state.lastErrorAt = new Date();
            state.lastErrorMessage = error?.message || `Failed to stream instrument profile for ${ticker}`;
            this.logger.error(state.lastErrorMessage, error?.stack);
        }
    }
    ...
    private async publishNews(ticker: string) {
        ...
        try {
            ...
            state.lastErrorAt = new Date();
            state.lastErrorMessage = error?.message || `Failed to stream news for ${ticker}`;
            this.logger.error(state.lastErrorMessage, error?.stack);
        }
    }
