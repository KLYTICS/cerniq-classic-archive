...
export class AnalyticsService {
     ...
     */
    async getSummary(organizationId: string, userId: string): Promise<AnalyticsSummary> {
    ...
    async getCategoryBreakdown(
        ...
        const grandTotal = categories.reduce((sum, c) => sum + Number(c._sum.amount || 0), 0);
    ...
    async getTeamComparison(
        ...
        return Array.from(teamMap.values()).sort((a, b) => b.totalAmount - a.totalAmount);
    }
