...
export class FactorRiskService {
     ...
     */
    async calculateFactorExposures(positions: PositionDto[]): Promise<FactorExposureResult> {
        ...
        return {
            ...
            totalFactorRisk: Object.values(factorRiskContributions).reduce((a, b) => a + b, 0),
    ...
    private estimateStockFactorExposures(ticker: string): Record<string, number> {
        ...
        if (techLarge.includes(ticker)) {
            exposures = { MKT: 1.15, SMB: -0.2, HML: -0.4, RMW: 0.3, CMA: 0.1, MOM: 0.2 };
        } else if (techSmall.includes(ticker)) {
            exposures = { MKT: 1.3, SMB: 0.5, HML: -0.5, RMW: -0.2, CMA: -0.3, MOM: 0.1 };
        } else if (valueStocks.includes(ticker)) {
            exposures = { MKT: 0.9, SMB: -0.1, HML: 0.5, RMW: 0.4, CMA: 0.3, MOM: -0.1 };
        } else if (growthStocks.includes(ticker)) {
            exposures = { MKT: 1.4, SMB: 0.1, HML: -0.6, RMW: -0.3, CMA: -0.4, MOM: 0.3 };
        } else if (smallCap.includes(ticker)) {
            exposures = { MKT: 1.5, SMB: 0.8, HML: 0.1, RMW: -0.4, CMA: -0.2, MOM: 0.4 };
        }
    ...
    private getFactorCovariance(): number[][] {
        ...
        return [
            [0.0400, 0.0020, 0.0010, 0.0005, 0.0005, -0.0010], // MKT
            [0.0020, 0.0144, 0.0015, 0.0010, 0.0005, 0.0002],  // SMB
            [0.0010, 0.0015, 0.0121, -0.0020, 0.0030, -0.0025], // HML
            [0.0005, 0.0010, -0.0020, 0.0064, 0.0015, 0.0010],  // RMW
            [0.0005, 0.0005, 0.0030, 0.0015, 0.0049, -0.0008],  // CMA
            [-0.0010, 0.0002, -0.0025, 0.0010, -0.0008, 0.0196], // MOM
        ];
    }
    ...
    private calculateFactorRiskContributions(
        ...
        for (let i = 0; i < 6; i++) {
            ...
            contributions[factors[i]] = (betas[i] * marginalContrib / portfolioVariance) * 100;
        }
     ...
     */
    private estimateIdiosyncraticRisk(stockExposures: StockFactorExposure[]): number {
        // Approximate idiosyncratic risk based on concentration
        const herfindahl = stockExposures.reduce((sum, s) => sum + s.weight * s.weight, 0);
        const effectiveStocks = 1 / herfindahl;

        // Average idiosyncratic vol ~30%, diversified by sqrt(N)
        const avgIdioVol = 0.30;
    ...
    async decomposeReturns(
        ...
        const factorReturns: Record<string, number> = {
            MKT: returnPeriod === 'daily' ? 0.05 : returnPeriod === 'weekly' ? 0.35 : 1.2,
            SMB: returnPeriod === 'daily' ? 0.02 : returnPeriod === 'weekly' ? 0.1 : 0.3,
            HML: returnPeriod === 'daily' ? -0.01 : returnPeriod === 'weekly' ? -0.05 : -0.15,
            RMW: returnPeriod === 'daily' ? 0.01 : returnPeriod === 'weekly' ? 0.08 : 0.25,
            CMA: returnPeriod === 'daily' ? 0.005 : returnPeriod === 'weekly' ? 0.03 : 0.1,
            MOM: returnPeriod === 'daily' ? 0.03 : returnPeriod === 'weekly' ? 0.2 : 0.6,
        };
