...
export class VolatilityController {
     ...
     */
    @Get('cone/:ticker')
    async getVolatilityCone(@Param('ticker') ticker: string): Promise<VolatilityConeResponseDto> {
        try {
            return await this.volatilityService.getVolatilityCone(ticker.toUpperCase());
        } catch (error: any) {
     ...
     */
    @Get('heatmap/:ticker')
    async getVolatilityHeatmap(@Param('ticker') ticker: string): Promise<VolatilityHeatmapResponseDto> {
        try {
            return await this.volatilityService.getVolatilityHeatmap(ticker.toUpperCase());
        } catch (error: any) {
            throw new HttpException(
    ...
    async getRealizedVsImplied(
        ...
        try {
            ...
            return await this.volatilityService.getRealizedVsImplied(ticker.toUpperCase(), numDays);
        } catch (error: any) {
            throw new HttpException(
    ...
    async getVolatilityStats(
        ...
        try {
            return await this.volatilityService.getVolatilityStats(ticker.toUpperCase(), period || '30d');
        } catch (error: any) {
            throw new HttpException(
