...
export class StressTestService {
    ...
    private readonly historicalScenarios: StressScenario[] = [
        ...
        },
        {
            ...
            shocks: { equity: -0.15, volatility: 1.8, rates: -0.01, fx: -0.20 },
        },
        {
        ...
        },
        {
            ...
            shocks: { equity: -0.50, volatility: 3.0, rates: -0.02, spreads: 0.05 },
        },
        {
    ...
    async runStressTest(
        ...
        for (const scenario of scenarios) {
            ...
            for (const pos of positionDetails) {
                ...
                pos.stressedValues.push({
                    ...
                    pnlPercent: ((stressedValue - pos.currentValue) / pos.currentValue) * 100,
                });
            }
        ...
        return {
            ...
            averageLoss: scenarioResults.reduce((sum, s) => sum + s.pnl, 0) / scenarioResults.length,
            timestamp: new Date(),
        };
    }
        ...
        return [
            {
                ...
                shocks: { equity: -0.10, volatility: 1.3 },
            },
            {
                name: 'Moderate Bear Market',
                description: '20% decline',
                shocks: { equity: -0.20, volatility: 1.8 },
            },
            {
            ...
            },
            {
                ...
                shocks: { equity: -0.50, volatility: 3.5 },
            },
            {
                name: 'Rate Spike (+200bps)',
                description: 'Sudden rate increase',
                shocks: { equity: -0.15, bonds: -0.10, rates: 0.02 },
            },
            {
                name: 'Stagflation',
     ...
     */
    private categorizeSeverity(pnlPercent: number): 'LOW' | 'MODERATE' | 'HIGH' | 'SEVERE' | 'CATASTROPHIC' {
    ...
    private findHistoricalParallel(equityShock: number): string {
        ...
        if (shock < 0.25) return 'Similar to Asian Crisis (1997) or Rate Shock (2022)';
        if (shock < 0.40) return 'Similar to COVID Crash (2020) or Dot-Com Crash';
    ...
    private estimateProbability(equityShock: number): string {
        const shock = Math.abs(equityShock);
        if (shock < 0.10) return '~20% per year (common)';
        if (shock < 0.20) return '~5% per year (once per decade)';
...
export type { StressScenario, StressTestResult, ReverseStressResult, PositionDto };
