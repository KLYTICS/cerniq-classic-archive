...
export class VolatilityService {
    ...
    async getVolatilityCone(ticker: string): Promise<VolatilityConeResponseDto> {
        ...
        if (!historicalData || historicalData.length < 30) {
            throw new Error('Insufficient historical data for volatility cone calculation');
        }
     ...
     */
    async getVolatilityHeatmap(ticker: string): Promise<VolatilityHeatmapResponseDto> {
     ...
     */
    async getRealizedVsImplied(ticker: string, days: number = 90): Promise<RealizedVsImpliedResponseDto> {
     ...
     */
    async getVolatilityStats(ticker: string, period: string = '30d'): Promise<VolatilityStatsDto> {
     ...
     */
    private async fetchHistoricalData(ticker: string, days: number): Promise<{ date: Date; close: number }[]> {
        const endDate = new Date();
        const startDate = new Date();
        startDate.setDate(startDate.getDate() - days);

        const historicalPrices = await this.marketDataService.getHistoricalPrices(ticker, startDate, endDate);
     ...
     */
    private calculateConePoint(returns: number[], horizon: number): VolatilityConePoint {
