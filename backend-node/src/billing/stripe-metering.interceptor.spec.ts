import { StripeMeteringInterceptor, getUsageLog } from './stripe-metering.interceptor';
import { of } from 'rxjs';
...
describe('StripeMeteringInterceptor', () => {
  ...
  const createContext = (method: string, path: string, params: Record<string, string> = {}, headers: Record<string, string> = {}): ExecutionContext =>
    ({
      switchToHttp: () => ({
        getRequest: () => ({ method, url: path, route: { path }, params, headers }),
  ...
  it('records compute_job usage for billable ALM endpoints', (done) => {
    ...
    const ctx = createContext('POST', '/api/alm/:id/monte-carlo/run', { institutionId: 'inst-1' });
  ...
  it('records api_call usage for /api/v1/ endpoints', (done) => {
    const before = Date.now();
    const ctx = createContext('GET', '/api/v1/some-resource', {}, { 'x-institution-id': 'inst-2' });
