...
export const StressScenarioResultSchema = z.object({
  ...
  (s) => s.classification === 'PASS' || (s.mitigation && s.mitigation.length > 0),
  { message: 'WARN/FAIL scenarios must include mitigation', path: ['mitigation'] },
);
...
export const StressTestOutputSchema = z.object({
  ...
  worstCase: z.object({
    ...
    (w) => w.classification !== 'FAIL' || (w.actionPlan && w.actionPlan.length > 0),
    { message: 'FAIL worst-case must include actionPlan', path: ['actionPlan'] },
  ),
...
export const COSSEC_SCENARIO_IDS = [
  ...
  'steepening_200', 'flattening_200', 'inversion_150',
  'pr_hurricane_scenario', 'pr_recession_scenario', 'pr_liquidity_crisis',
] as const;
