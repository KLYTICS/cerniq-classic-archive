  ...
  onBalanceSheetUploaded(ev: BalanceSheetUploadedEvent): void {
    this.runner
      .run({
        ...
        input: { institutionId: ev.institutionId, region: 'PR', language: 'bilingual' },
      })
      .catch((err: Error) => this.logger.error(`ALM_DECISION failed for ${ev.balanceSheetId}`, err));
  }

  async runScheduledMonitor(institutionId: string, scanKind: 'daily' | 'weekly' | 'monthly', organizationId?: string | null) {
    ...
    return this.runner.run({
      ...
      idempotencyKey: deriveKey('RISK_MONITOR', `${scanKind}:${institutionId}`, today),
...
function deriveKey(a: string, b: string, c: string): string {
  return createHash('sha256').update(`${a}|${b}|${c}`).digest('hex').slice(0, 32);
}
