...
function makeHistogram(buckets: number[]): { buckets: HistogramBucket[]; sum: number; count: number } {
  return { buckets: buckets.map((le) => ({ le, count: 0 })), sum: 0, count: 0 };
}

function observeHistogram(h: ReturnType<typeof makeHistogram>, value: number): void {
  h.sum += value;
  h.count++;
...
export class AgentMetricsService implements OnModuleInit {
  ...
  private readonly runDuration = makeHistogram([1_000, 5_000, 10_000, 30_000, 45_000, 90_000, 180_000]);
  private readonly toolDuration = makeHistogram([100, 500, 1_000, 5_000, 10_000, 30_000, 60_000]);
  private readonly llmTokensIn = makeHistogram([500, 1_000, 2_000, 4_000, 8_000, 16_000, 32_000]);
  private readonly llmTokensOut = makeHistogram([100, 500, 1_000, 2_000, 4_000, 8_000]);
  ...
  private readonly byAgent = new Map<string, { started: number; succeeded: number; failed: number; totalMs: number }>();
    ...
    this.eventBus.onAny((event: any) => {
      switch (event?.type) {
        ...
        case 'RUN_COMPLETED': {
          ...
          if (event.inputTokens != null) observeHistogram(this.llmTokensIn, event.inputTokens);
          if (event.outputTokens != null) observeHistogram(this.llmTokensOut, event.outputTokens);
          if (event.costUsdCents != null) this.counters.costUsdCents += event.costUsdCents;
          break;
        }
        case 'RUN_FAILED':
        ...
        case 'RUN_STEP':
          if (event.stepKind === 'TOOL_RESULT') {
            ...
            if (event.durationMs != null) observeHistogram(this.toolDuration, event.durationMs);
  ...
  private ensureAgent(id: string) {
    if (!this.byAgent.has(id)) this.byAgent.set(id, { started: 0, succeeded: 0, failed: 0, totalMs: 0 });
...
export interface AgentMetricsSnapshot {
  counters: {
    runsStarted: number; runsSucceeded: number; runsFailed: number; runsTimedOut: number;
    toolCalls: number; toolErrors: number; alertsEmitted: number; alertsCritical: number; costUsdCents: number;
  };
  histograms: {
    runDurationMs: { p50: number | null; p95: number | null; p99: number | null; count: number; sum: number };
    toolDurationMs: { p50: number | null; p95: number | null; p99: number | null; count: number; sum: number };
    llmTokensIn: { p50: number | null; p95: number | null; p99: number | null; count: number; sum: number };
    llmTokensOut: { p50: number | null; p95: number | null; p99: number | null; count: number; sum: number };
  };
  byAgent: Record<string, { started: number; succeeded: number; failed: number; totalMs: number }>;
  collectedAt: string;
}
...
function percentile(h: ReturnType<typeof makeHistogram>, p: number): number | null {
