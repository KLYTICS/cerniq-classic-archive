...
import type { GovernedScenarioSeed, GovernedBenchmarkSeed } from './governance.types';
...
const SCENARIOS: GovernedScenarioSeed[] = [
  // ── Regulatory mandated ──
  {
    ...
    description: 'COSSEC-mandated baseline stress scenario for 2026 examination cycle. +200bps parallel shift, 5% deposit runoff.',
    version: '1.0.0',
    scope: 'REGULATORY',
    status: 'APPROVED',
    ...
    provenance: { regulatoryReference: 'COSSEC Examination Manual §4.3', effectiveDate: '2026-01-01' },
  },
  {
    scenarioKey: 'stress.cossec-adverse-2026',
    displayName: 'COSSEC Adverse Stress 2026',
    description: 'COSSEC adverse scenario: +300bps short / +100bps long (steepening), 10% deposit runoff, 2% credit losses.',
    version: '1.0.0',
    scope: 'REGULATORY',
    status: 'APPROVED',
    source: 'COSSEC_2026_EXAM_GUIDE',
  ...
  },
  {
    ...
    description: 'Federal Reserve severely adverse scenario for CCAR/DFAST. Deep recession with rate drop, high unemployment, asset price decline.',
    version: '1.0.0',
    scope: 'REGULATORY',
    status: 'APPROVED',
    ...
    approvedUses: ['regulatory_filing', 'board_report'],
    provenance: { regulatoryReference: 'FRB 2026 Supervisory Scenarios', publishedDate: '2026-02-15' },
  },
  // ── PR-specific ──
  {
    scenarioKey: 'stress.hurricane-category-4',
    displayName: 'Hurricane Category 4 (PR-Specific)',
    description: 'Maria-calibrated Cat 4 hurricane: 20% deposit surge (pre-storm withdrawals), 8% credit losses on property-secured loans, 30-day liquidity stress.',
    version: '1.0.0',
    scope: 'SECTOR',
    status: 'APPROVED',
    ...
    provenance: { basis: 'Hurricane Maria 2017 post-mortem analysis', calibratedDate: '2024-06-15' },
  },
  {
    scenarioKey: 'stress.rapid-rate-rise-300',
    displayName: 'Rapid Rate Rise +300bps',
    description: 'Parallel +300bps shock over 6 months. Tests NII sensitivity and deposit beta assumptions.',
    version: '1.0.0',
    scope: 'SECTOR',
    status: 'APPROVED',
    source: 'INTERNAL_RISK_COMMITTEE',
  ...
  },
  {
    ...
    description: 'Short rates +200bps, long rates -100bps. Tests NIM compression and funding cost sensitivity.',
    version: '1.0.0',
    scope: 'SECTOR',
    status: 'APPROVED',
    source: 'INTERNAL_RISK_COMMITTEE',
...
const BENCHMARKS: GovernedBenchmarkSeed[] = [
  {
    ...
    description: 'US Treasury constant maturity par yields as of March 31, 2026.',
    ...
    provenance: { sourceUrl: 'https://www.federalreserve.gov/releases/h15/', extractedDate: '2026-04-01' },
    fallbackPolicy: 'use_prior_version',
  },
  {
    datasetKey: 'peer.pr-cooperativa-100m-250m',
    displayName: 'PR Cooperativa Peer Group ($100M–$250M)',
    description: 'Peer benchmarks for Puerto Rico cooperativas with $100M–$250M total assets. Based on COSSEC filings.',
    ...
    data: {
      ...
      metrics: {
        ...
        delinquencyRatio: { p25: 1.2, p50: 2.1, p75: 3.5, mean: 2.3, unit: '%' },
        roa: { p25: 0.4, p50: 0.7, p75: 1.0, mean: 0.7, unit: '%' },
        operatingExpenseRatio: { p25: 3.2, p50: 3.8, p75: 4.5, mean: 3.9, unit: '%' },
  ...
  },
  {
    ...
    description: 'Minimum and target thresholds for COSSEC-regulated institutions. 12 ratios.',
    ...
    data: {
      thresholds: [
        { ratio: 'Net Worth Ratio', minimum: 6, target: 7, unit: '%', reference: 'COSSEC §3.1' },
        { ratio: 'Interest Margin', minimum: 2, target: 3, unit: '%', reference: 'COSSEC §3.2' },
        { ratio: 'Operating Expense Ratio', maximum: 5, target: 4, unit: '%', reference: 'COSSEC §3.3' },
        { ratio: 'Delinquency Ratio', maximum: 5, target: 3, unit: '%', reference: 'COSSEC §3.4' },
        { ratio: 'ROA', minimum: 0.25, target: 0.5, unit: '%', reference: 'COSSEC §3.5' },
        { ratio: 'Loan to Asset Ratio', minimum: 50, target: 65, unit: '%', reference: 'COSSEC §3.6' },
      ],
    },
    provenance: { regulatoryDocument: 'COSSEC Examination Manual 2026 Edition' },
...
export class GovernanceSeeder implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    for (const s of SCENARIOS) {
      try {
        const existing = await this.scenarioService.getByKey(s.scenarioKey).catch(() => null);
        await this.scenarioService.upsert(s);
        if (!existing) sCreated++;
      } catch (err: any) {
        this.logger.warn(`Failed to seed scenario ${s.scenarioKey}: ${err.message}`);
      }
    }
    this.logger.log(`Governed scenarios seeded: ${sCreated} created, ${SCENARIOS.length} total`);

    // Seed benchmarks
    let bCreated = 0;
    for (const b of BENCHMARKS) {
      try {
        const existing = await this.benchmarkService.getByKey(b.datasetKey).catch(() => null);
        await this.benchmarkService.upsert(b);
        if (!existing) bCreated++;
      } catch (err: any) {
        this.logger.warn(`Failed to seed benchmark ${b.datasetKey}: ${err.message}`);
      }
    }
    this.logger.log(`Governed benchmarks seeded: ${bCreated} created, ${BENCHMARKS.length} total`);
  }
}
