...
const requestLatencies: Array<{ endpoint: string; method: string; durationMs: number; statusCode: number; timestamp: number }> = [];
...
export class MetricsService {
  ...
  recordRequest(endpoint: string, method: string, durationMs: number, statusCode: number) {
    requestLatencies.push({ endpoint, method, durationMs, statusCode, timestamp: Date.now() });
  ...
  async getSystemHealth(): Promise<SystemHealth> {
    ...
    const apiMetrics: APIMetrics[] = Array.from(byEndpoint.entries()).map(([key, reqs]) => {
      ...
      return {
        ...
        errorRate: reqs.length > 0 ? Math.round((errors.length / reqs.length) * 10000) / 100 : 0,
      };
    }).sort((a, b) => b.requestCount - a.requestCount).slice(0, 20);

    const businessMetrics = await this.getBusinessMetrics();

    const totalErrors = recentRequests.filter(r => r.statusCode >= 500).length;
    const errorRate = recentRequests.length > 0 ? totalErrors / recentRequests.length : 0;
    const p99 = apiMetrics.length > 0 ? Math.max(...apiMetrics.map(m => m.p99Ms)) : 0;

    return {
      status: errorRate > 0.05 ? 'unhealthy' : p99 > 5000 ? 'degraded' : 'healthy',
  ...
  private async getBusinessMetrics(): Promise<BusinessMetrics> {
    ...
    const [institutions, users, todayUsage] = await Promise.all([
      ...
      this.prisma.usageMeterEvent.count({ where: { createdAt: { gte: today } } }).catch(() => 0),
    ]);
