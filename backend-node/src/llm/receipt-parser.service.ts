...
If any field is not clearly visible, omit it or use null. Be precise with numbers.`;
    ...
    async checkPolicyViolations(
        ...
        if (parsedReceipt.amount > 1000) {
            violations.push('Expense exceeds $1,000 limit - requires manager approval');
        }

        if (parsedReceipt.category === 'Meals & Entertainment' && parsedReceipt.amount > 100) {
        ...
        if (parsedReceipt.items && parsedReceipt.items.length > 0) {
            const itemsList = parsedReceipt.items.map((i) => i.description).join(', ');
    ...
    async checkDuplicate(
        ...
        return existingReceipts.some(
            (existing) =>
                existing.merchantName.toLowerCase() === newReceipt.merchantName.toLowerCase() &&
