...
import { PortfolioDto, PositionDto, CreatePortfolioDto, UpdatePortfolioDto, AddPositionDto, PortfolioAnalyticsDto } from './dto/portfolio.dto';
...
export class PortfolioService {
     ...
     */
    async getPortfolio(portfolioId: string, userId: string): Promise<PortfolioDto> {
     ...
     */
    async createPortfolio(userId: string, createDto: CreatePortfolioDto): Promise<PortfolioDto> {
     ...
     */
    async updatePortfolio(portfolioId: string, userId: string, updateDto: UpdatePortfolioDto): Promise<PortfolioDto> {
        // defined check first
        const existing = await this.prisma.portfolio.findUnique({ where: { id: portfolioId } });
     ...
     */
    async addPosition(portfolioId: string, userId: string, addDto: AddPositionDto): Promise<PositionDto> {
        ...
        if (existing) {
            ...
            const totalCost = (Number(existing.avgCost) * Number(existing.quantity)) + (addDto.price * addDto.quantity);
        ...
        this.logger.log(`Updated position ${addDto.ticker} in portfolio ${portfolioId}`);
     ...
     */
    async removePosition(portfolioId: string, userId: string, ticker: string, quantity: number, sellPrice: number): Promise<void> {
        const portfolio = await this.prisma.portfolio.findUnique({ where: { id: portfolioId } });
        if (!portfolio || portfolio.userId !== userId) {
            throw new NotFoundException(`Portfolio ${portfolioId} not found`);
        }
     ...
     */
    async getPortfolioAnalytics(portfolioId: string, userId: string): Promise<PortfolioAnalyticsDto> {
        ...
        const sortedPositions = [...dto.positions].sort((a, b) => b.unrealizedPnLPercent - a.unrealizedPnLPercent);
        const bestPerformer = sortedPositions[0] ? { ticker: sortedPositions[0].ticker, return: sortedPositions[0].unrealizedPnLPercent } : { ticker: 'N/A', return: 0 };
        const worstPerformer = sortedPositions[sortedPositions.length - 1] ? { ticker: sortedPositions[sortedPositions.length - 1].ticker, return: sortedPositions[sortedPositions.length - 1].unrealizedPnLPercent } : { ticker: 'N/A', return: 0 };
        ...
        return {
            ...
            beta: 1.0,
            maxDrawdown: 0,
            winRate: dto.positions.filter(p => p.unrealizedPnL > 0).length / (dto.positions.length || 1),
            bestPerformer,
            worstPerformer,
        };
    }
    ...
    private async updatePortfolioValues(portfolio: PortfolioDto): Promise<void> {
        ...
        for (const position of portfolio.positions) {
            try {
                ...
                position.unrealizedPnLPercent = position.avgCost > 0 ? (position.unrealizedPnL / (position.avgCost * position.quantity)) * 100 : 0;
        ...
        portfolio.totalPnLPercent = portfolio.initialCash > 0 ? (portfolio.totalPnL / portfolio.initialCash) * 100 : 0;

        // Update position weights
        for (const position of portfolio.positions) {
            position.weight = portfolio.totalValue > 0 ? (position.marketValue / portfolio.totalValue) * 100 : 0;
        }
    }
}
