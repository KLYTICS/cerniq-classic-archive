...
export class ValuationService {
    ...
    async getValuation(request: ValuationRequestDto): Promise<any> {
        ...
        try {
            ...
            this.logger.warn(`Fundamentals not available for ${ticker}, using defaults`);
            fundamentals = {};
        }

        // Auto-detect valuation type if not specified
        const selectedType = valuationType === 'auto'
        ...
        switch (selectedType) {
            ...
            case 'compounder':
                return this.compounderEngine.calculate(ticker, quote.price, fundamentals);
            case 'frontier':
    ...
    async getKPIScore(ticker: string): Promise<KPIScoreDto> {
        ...
        return this.kpiEngine.calculate(ticker, fundamentals, { price: quote.price });
    }
    ...
    async runScreener(request: ScreenerRequestDto): Promise<ScreenerResultDto[]> {
        ...
        for (const tickerData of tickersResponse.tickers.slice(0, 20)) {
            try {
                ...
                const valuationType = request.valuationType || this.detectValuationType(tickerData.ticker, {});
                ...
                results.push({
                    ...
                    fairValue: valuation.fairValue || valuation.probabilityWeightedValue || 0,
                ...
                this.logger.warn(`Failed to value ${tickerData.ticker}: ${error.message}`);
            }
        }
     ...
     */
    private detectValuationType(ticker: string, fundamentals: any): 'cyclical' | 'compounder' | 'frontier' {
        // Industry-based detection (simplified)
        const cyclicalSectors = ['Materials', 'Energy', 'Industrials'];
        const compounderSectors = ['Technology', 'Consumer Discretionary', 'Healthcare'];

        // Check ticker patterns
        if (ticker.includes('SEMI') || ticker.includes('ASML') || ticker.includes('LRCX')) {
            return 'cyclical'; // Semiconductor equipment
        }

        if (ticker.includes('AI') || ticker.includes('NVDA') || fundamentals?.sector === 'AI') {
