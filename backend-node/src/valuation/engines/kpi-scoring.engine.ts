...
export class KPIScoringEngine {
     ...
     */
    async calculate(ticker: string, fundamentals: any, marketData: any): Promise<KPIScoreDto> {
    ...
    private scoreROIC(fundamentals: any): number {
        const roic = 0.20; // 20% placeholder
        return Math.min((roic / 0.30) * 100, 100);
    }
    ...
    private scoreFCFYield(fundamentals: any): number {
        ...
        return Math.min((fcfYield / 0.10) * 100, 100);
    }
