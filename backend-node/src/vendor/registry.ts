...
export const VENDOR_REGISTRY: ReadonlyArray<VendorEntry> = [
  ...
  {
    id: 'bloomberg-bpipe',
    name: 'Bloomberg (HAPI variant)',
    description:
      'Institutional-grade market data via Bloomberg HAPI REST (cerniq targets HAPI over BPIPE binary for headless server-side use)',
    category: 'macro-rates',
    status: 'beta',
    compliancePosture: 'paid-enterprise',
    integrationCostDays: 7,
    url: 'https://www.bloomberg.com/professional/product/market-data/',
    providerPath: 'vendor/market-data/bloomberg-hapi.provider.ts',
    envVars: [
      'BLOOMBERG_HAPI_BASE_URL',
      'BLOOMBERG_HAPI_CLIENT_ID',
      'BLOOMBERG_HAPI_CLIENT_SECRET',
      'BLOOMBERG_HAPI_ACCOUNT_ID',
      'BLOOMBERG_HAPI_ENABLE_ASYNC_POLL',
    ],
    prCooperativaRelevance:
      'Tier-1 institutional rate source — most PR cooperativas do not use Bloomberg directly; useful if cerniq grows upmarket. Cerniq targets HAPI (REST) over BPIPE/BLPAPI binary so we can run headless server-side.',
    blockedBy:
      'Bloomberg EDL contract + HAPI account provisioning required. HAPI client credentials needed to flip BLOOMBERG_HAPI_ENABLE_ASYNC_POLL to true.',
  },
  {
    id: 'refinitiv-eikon',
    name: 'Refinitiv Eikon',
    description:
      'LSEG Refinitiv Data Platform (RDP) — pricing snapshots, news, reference data over REST',
    category: 'macro-rates',
    status: 'beta',
    compliancePosture: 'paid-enterprise',
    integrationCostDays: 5,
    url: 'https://www.lseg.com/en/data-analytics',
    providerPath: 'vendor/market-data/refinitiv-eikon.provider.ts',
    envVars: ['REFINITIV_APP_KEY', 'REFINITIV_APP_SECRET'],
    prCooperativaRelevance:
      'Tier-1 alternative to Bloomberg; some cooperativas have Refinitiv via their broker relationships. Full OAuth2 + REST call path wired; ready for production the moment paid LSEG credentials land.',
    blockedBy:
      'LSEG paid contract with redistribution license clarity. App key + secret provisioned via LSEG Workspace developer portal.',
  },
  {
    id: 'ice-bofa',
