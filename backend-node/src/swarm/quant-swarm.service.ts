...
export class QuantSwarmService {
  ...
  async runFullSwarm(institutionId: string, services: {
    yieldCurve: any; liquidity: any; cecl: any; concentration: any;
    ftp: any; peers: any; camel: any; climate: any; advisor: any;
    ...
    const modelNames = ['rateShock', 'liquidity', 'cecl', 'concentration', 'ftp', 'peers', 'camel', 'climate'];
    const allResults = [rateShock, liquidity, cecl, concentration, ftp, peers, camel, climate];

    const completed = modelNames.filter((_, i) => allResults[i].status === 'fulfilled');
    const failed = modelNames.filter((_, i) => allResults[i].status === 'rejected');

    if (failed.length > 0) {
      this.logger.warn(`Swarm: ${failed.length} models failed: ${failed.join(', ')}`);
    }

    const healthScore = await services.advisor.computeHealthScore(institutionId).catch(() => ({ overall: 50 }));
