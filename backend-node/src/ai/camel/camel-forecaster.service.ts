...
export class CamelForecasterService {
  ...
  async forecastForInstitution(institutionId: string): Promise<CAMELForecast[]> {
    // Load CAMEL history (from board reports or computed snapshots)
    const reports = await this.prisma.boardReport.findMany({
      where: { institutionId },
      orderBy: { generatedAt: 'asc' },
      take: 8,
      select: { camelComposite: true, nimSnapshot: true, lcrSnapshot: true, generatedAt: true },
    });

    // If insufficient history, generate synthetic from current state
    const dimensions = ['capital', 'assetQuality', 'management', 'earnings', 'liquidity'] as const;
    const forecasts: CAMELForecast[] = [];

    for (const dim of dimensions) {
      const series = this.getDimensionSeries(reports, dim);
      const params = this.fitAR2(series);
      const last2: [number, number] = [series[series.length - 2] ?? series[0] ?? 2.5, series[series.length - 1] ?? 2.5];
      ...
      forecasts.push({
        ...
        trend: q4 < current - 0.25 ? 'improving' : q4 > current + 0.25 ? 'deteriorating' : 'stable',
        ar2Params: params,
      });
    }
  ...
  private fitAR2(series: number[]): { phi1: number; phi2: number; intercept: number; r2: number } {
    if (series.length < 4) return { phi1: 0, phi2: 0, intercept: series[series.length - 1] ?? 2.5, r2: 0 };
    ...
    const XtX = Xt.map(row => X[0].map((_, j) => row.reduce((s, _, k) => s + row[k] * X[k][j], 0)));
    ...
    return { phi1, phi2, intercept, r2: ssTot > 0 ? Math.max(0, 1 - ssRes / ssTot) : 0 };
  }

  private forecastN(params: { phi1: number; phi2: number; intercept: number }, last2: [number, number], h: number): number {
  ...
  private residualStd(series: number[], params: { phi1: number; phi2: number; intercept: number }): number {
    const residuals: number[] = [];
    for (let t = 2; t < series.length; t++) {
      residuals.push(series[t] - (params.intercept + params.phi1 * series[t - 1] + params.phi2 * series[t - 2]));
    }
    if (residuals.length === 0) return 0.5;
    const mean = residuals.reduce((a, b) => a + b, 0) / residuals.length;
    return Math.sqrt(residuals.reduce((s, r) => s + (r - mean) ** 2, 0) / residuals.length);
  }
  ...
  private solve3x3(A: number[][], b: number[]): number[] {
    ...
    const replace = (col: number) => A.map((row, i) => [...row.slice(0, col), b[i], ...row.slice(col + 1)]);
