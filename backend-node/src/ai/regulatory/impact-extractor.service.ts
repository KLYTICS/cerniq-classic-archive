...
export class ImpactExtractorService {
  ...
  async extract(publicationId: string): Promise<RegulatoryImpact> {
    const pub = await this.prisma.regulatoryPublication.findUniqueOrThrow({ where: { id: publicationId } });
    ...
    if (process.env.ANTHROPIC_API_KEY) {
      try {
        ...
        const response = await client.messages.create({
          ...
          max_tokens: 800,
          system: 'Eres un experto en regulación financiera de Puerto Rico. Responde SOLO con JSON válido.',
          messages: [{
            role: 'user',
  ...
  private heuristicExtract(pub: any): RegulatoryImpact {
    ...
    if (text.includes('liquidez') || text.includes('lcr') || text.includes('liquidity')) affectedSubcategories.push('liquidity');
    if (text.includes('capital') || text.includes('nwr') || text.includes('net worth')) affectedSubcategories.push('capital');
    if (text.includes('tasa') || text.includes('interés') || text.includes('rate')) affectedSubcategories.push('interest_rate');
    if (text.includes('préstamo') || text.includes('loan') || text.includes('crédito')) affectedSubcategories.push('credit');
    if (text.includes('concentración') || text.includes('concentration')) affectedSubcategories.push('concentration');

    const severity = text.includes('obligat') || text.includes('mandatory') || text.includes('inmediato')
      ? 'HIGH' : text.includes('recomend') || text.includes('guideline') ? 'LOW' : 'MEDIUM';
