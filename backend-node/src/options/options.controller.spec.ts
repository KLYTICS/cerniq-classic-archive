...
describe('OptionsController', () => {
  ...
  it('calculateGreeks throws HttpException on service error', async () => {
    mockOptionsService.calculateGreeks.mockRejectedValue(
      new Error('Negative volatility'),
    );

    await expect(controller.calculateGreeks({} as any)).rejects.toThrow(
      'Negative volatility',
    );
  });
  ...
  it('getOptionsChain throws HttpException on service error', async () => {
    mockOptionsService.getOptionsChain.mockRejectedValue(
      new Error('Ticker not found'),
    );

    await expect(controller.getOptionsChain('BAD')).rejects.toThrow(
      'Ticker not found',
    );
  });
  ...
  it('calculateImpliedVolatility delegates to OptionsService', async () => {
    ...
    expect(result).toEqual(expected);
    expect(mockOptionsService.calculateImpliedVolatility).toHaveBeenCalledWith(
      dto,
    );
  });

  it('calculateImpliedVolatility throws HttpException on service error', async () => {
    mockOptionsService.calculateImpliedVolatility.mockRejectedValue(
      new Error('Convergence failed'),
    );

    await expect(
      controller.calculateImpliedVolatility({} as any),
  ...
  it('calculateStrategy throws HttpException on service error', async () => {
    mockOptionsService.calculateStrategy.mockRejectedValue(
      new Error('Invalid legs'),
    );

    await expect(controller.calculateStrategy({} as any)).rejects.toThrow(
      'Invalid legs',
    );
  });
  ...
  it('getVolatilitySurface delegates to OptionsService', async () => {
    ...
    expect(result).toEqual(expected);
    expect(mockOptionsService.getVolatilitySurface).toHaveBeenCalledWith(
      'AAPL',
    );
  });

  it('getVolatilitySurface throws HttpException on service error', async () => {
    mockOptionsService.getVolatilitySurface.mockRejectedValue(
      new Error('No data'),
    );

    await expect(controller.getVolatilitySurface('BAD')).rejects.toThrow(
      'No data',
    );
  });
});
