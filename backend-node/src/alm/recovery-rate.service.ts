...
export class RecoveryRateService {
  estimate(
    ...
    portfolioLGD: number;
    portfolioRecovery: number;
    interpretation: string;
    interpretationEs: string;
  } {
    const enriched = loans.map((l) => {
      const ltv = +(
        (l.balance / (l.collateralValue || l.balance)) *
        100
      ).toFixed(1);
      ...
      const estimatedRecovery = +(
        baseRecovery *
        seniorityAdj *
        Math.min(1, l.collateralValue / l.balance)
      ).toFixed(2);
      return {
        ...l,
        ltv,
        estimatedRecovery,
        lgd: +(1 - estimatedRecovery).toFixed(2),
      };
    });
    const totalBalance = enriched.reduce((s, l) => s + l.balance, 0);
    const portfolioLGD = +(
      enriched.reduce((s, l) => s + l.lgd * l.balance, 0) / totalBalance
    ).toFixed(3);
    return {
      loans: enriched,
      portfolioLGD,
      portfolioRecovery: +(1 - portfolioLGD).toFixed(3),
