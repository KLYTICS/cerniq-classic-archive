...
export class NetWorthAtRiskService {
  ...
  calculateNWaR(params: {
    ...
    const { assets, liabilities, equity, durationGap, rateVolatility, confidence = 0.99 } = params;

    this.logger.log(`Computing NWaR: equity=${equity}, durationGap=${durationGap}, rateVol=${rateVolatility}`);
    ...
    const breakEvenShock = (assets * Math.abs(durationGap)) > 0
    ...
    return {
      ...
      breakEvenShock: breakEvenShock === Infinity ? Infinity : +breakEvenShock.toFixed(6),
  ...
  stressTestNWaR(params: {
    ...
    durationGap: number;
    rateShocks: number[];
  }): Array<{ rateShock: number; equityImpact: number; residualEquity: number; solvent: boolean }> {
