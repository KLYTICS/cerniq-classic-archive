...
export class ScenarioPnLService {
  attribute(params: {
    ...
    const factors = [
      {
        name: 'Interest Rate',
        nameEs: 'Tasa de Interes',
        amount: rateContribution,
        pct: +((rateContribution / totalPnL) * 100).toFixed(1),
      },
      {
        name: 'Credit Spread',
        nameEs: 'Spread Crediticio',
        amount: spreadContribution,
        pct: +((spreadContribution / totalPnL) * 100).toFixed(1),
      },
      {
        name: 'Credit Events',
        nameEs: 'Eventos Crediticios',
        amount: creditContribution,
        pct: +((creditContribution / totalPnL) * 100).toFixed(1),
      },
      {
        name: 'FX',
        nameEs: 'Tipo de Cambio',
        amount: fxContribution,
        pct: +((fxContribution / totalPnL) * 100).toFixed(1),
      },
      {
        name: 'Residual',
        nameEs: 'Residual',
        amount: +residual.toFixed(2),
        pct: +((residual / totalPnL) * 100).toFixed(1),
      },
    ];

    const dominant = factors.reduce((max, f) =>
      Math.abs(f.amount) > Math.abs(max.amount) ? f : max,
    );
