...
export class AlmService {
    ...
    durationGapAnalysis(balanceSheet: BalanceSheetDto): DurationGapResult {
        const assetDetails = balanceSheet.assets.map((i) => this.instrumentDetail(i));
        const liabilityDetails = balanceSheet.liabilities.map((i) => this.instrumentDetail(i));
        ...
        const liabilityDuration = this.weightedDuration(liabilityDetails, totalLiabilities);
    ...
    basisPointValue(balanceSheet: BalanceSheetDto): BPVResult {
        ...
        const totalAssetBPV = round(assetBPVs.reduce((s, b) => s + b.bpv, 0), 2);
        const totalLiabilityBPV = round(liabilityBPVs.reduce((s, b) => s + b.bpv, 0), 2);
    ...
    private computeLCR(hqla: HQLADto, totalNetOutflows: number): LCRResult {
        ...
        const lcr = totalNetOutflows > 0
            ? round((hqlaTotal / totalNetOutflows) * 100, 2)
            : 0;
    ...
    private niiRiskLevel(changePct: number): 'low' | 'medium' | 'high' | 'critical' {
        const abs = Math.abs(changePct);
        if (abs < 0.05) return 'low';        // < 5%
        if (abs < 0.10) return 'medium';      // 5–10%
        if (abs < 0.20) return 'high';        // 10–20%
        return 'critical';                     // ≥ 20%
    }
}
