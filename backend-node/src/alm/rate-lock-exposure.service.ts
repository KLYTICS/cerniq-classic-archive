...
export class RateLockExposureService {
  analyze(params: {
    locks: Array<{ type: string; amount: number; rate: number; daysToClose: number; pullThroughPct: number }>;
    currentMarketRate: number;
  }): {
    totalPipeline: number; expectedFunding: number;
    rateExposure: number; // dollar risk from rate move
    locks: Array<{ type: string; amount: number; rate: number; marketRate: number; gapBps: number; expectedAmount: number; exposure: number }>;
    ...
    const enriched = params.locks.map(l => {
      ...
      const exposure = +(expectedAmount * Math.abs(gapBps) / 10000 * (l.daysToClose / 365)).toFixed(0);
      return { ...l, marketRate: params.currentMarketRate, gapBps, expectedAmount, exposure };
    });
    ...
    return {
      totalPipeline, expectedFunding, rateExposure, locks: enriched,
