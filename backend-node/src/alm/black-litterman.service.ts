...
export interface BLResult {
  ...
  assetNames: string[];
  viewContributions: Array<{ view: string; priorReturn: number; posteriorReturn: number; shift: number }>;
...
export class BlackLittermanService {
  ...
  async computeBLPortfolio(institutionId: string, views: BLView[] = []): Promise<BLResult> {
    ...
    const totalBalance = Array.from(bySub.values()).reduce((s, v) => s + v.balance, 0);
    const marketWeights = assetNames.map(name => (bySub.get(name)?.balance ?? 0) / (totalBalance || 1));
    ...
    const covMatrix = vols.map((vi, i) => vols.map((vj, j) => i === j ? vi * vi : vi * vj * rho));
    ...
    const equilibriumReturns = this.matVec(this.scalarMat(covMatrix, delta), marketWeights);

    if (views.length === 0) {
      // No views — optimize on equilibrium
      const optWeights = this.mvoOptimize(equilibriumReturns, covMatrix, delta);
      const portRet = optWeights.reduce((s, w, i) => s + w * equilibriumReturns[i], 0);
      const portVar = this.quadForm(optWeights, covMatrix);
      return {
        equilibriumReturns, posteriorReturns: equilibriumReturns,
        optimalWeights: optWeights, assetNames,
        viewContributions: [],
    ...
    const omega = views.map((v, i) => P_tS_Pt[i]?.[i] ? P_tS_Pt[i][i] / (v.confidence || 0.5) : 0.01);
    const omegaMat = omega.map((o, i) => omega.map((_, j) => i === j ? o : 0));

    // Step 4: BL posterior
    const tauSigmaInv = this.pseudoInverse(tauSigma);
    const omegaInv = this.pseudoInverse(omegaMat);
    const PtOmegaInvP = this.matMul(this.matMul(this.transpose(P), omegaInv), P);
    ...
    const posteriorReturns = this.matVec(Minv, term1.map((v, i) => v + (term2[i] ?? 0)));

    // Step 5: MVO on posterior
    const optWeights = this.mvoOptimize(posteriorReturns, covMatrix, delta);
    const portRet = optWeights.reduce((s, w, i) => s + w * posteriorReturns[i], 0);
    const portVar = this.quadForm(optWeights, covMatrix);
    ...
    return {
      ...
      viewContributions: views.map((v, i) => ({
        view: v.description,
        priorReturn: +equilibriumReturns[assetNames.indexOf(v.assets[0]) ?? 0]?.toFixed(4),
        posteriorReturn: +posteriorReturns[assetNames.indexOf(v.assets[0]) ?? 0]?.toFixed(4),
        shift: +(posteriorReturns[assetNames.indexOf(v.assets[0]) ?? 0] - equilibriumReturns[assetNames.indexOf(v.assets[0]) ?? 0]).toFixed(4),
      })),
      portfolioExpectedReturn: +portRet.toFixed(4),
      portfolioRisk: +Math.sqrt(portVar).toFixed(4),
      sharpeRatio: portVar > 0 ? +(portRet / Math.sqrt(portVar)).toFixed(4) : 0,
  ...
  // ─── Matrix Utilities ─────────────────────────────────────
  private scalarMat(M: number[][], s: number): number[][] { return M.map(r => r.map(v => v * s)); }
  private addMat(A: number[][], B: number[][]): number[][] { return A.map((r, i) => r.map((v, j) => v + (B[i]?.[j] ?? 0))); }
  private transpose(M: number[][]): number[][] { return M[0]?.map((_, i) => M.map(r => r[i])) ?? []; }
  private matVec(M: number[][], v: number[]): number[] { return M.map(r => r.reduce((s, a, i) => s + a * (v[i] ?? 0), 0)); }
  private matMul(A: number[][], B: number[][]): number[][] { return A.map(r => (B[0] ?? []).map((_, j) => r.reduce((s, _, k) => s + r[k] * (B[k]?.[j] ?? 0), 0))); }
  private quadForm(w: number[], S: number[][]): number { return w.reduce((s, wi, i) => s + wi * w.reduce((s2, wj, j) => s2 + wj * (S[i]?.[j] ?? 0), 0), 0); }
  ...
  private mvoOptimize(mu: number[], cov: number[][], delta: number): number[] {
    ...
    const raw = this.matVec(covInv, mu.map(m => m / delta));
  ...
  private getDemoResult(): BLResult {
    return {
      ...
      optimalWeights: [0.10, 0.20, 0.25, 0.15, 0.20, 0.10],
      assetNames: ['cash', 'securities', 'residential_mortgage', 'commercial_re', 'consumer_loans', 'savings'],
      viewContributions: [],
      portfolioExpectedReturn: 0.052, portfolioRisk: 0.045, sharpeRatio: 1.15,
    };
  }
}
