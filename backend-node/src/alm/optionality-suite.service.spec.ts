...
describe('OptionalitySuiteService', () => {
  ...
  describe('analyzePortfolio with real items', () => {
    ...
    beforeEach(() => {
      const items = [
        { id: '1', name: 'Residential Mortgages', category: 'asset', subcategory: 'residential_mortgage', balance: 100, duration: 5, rate: 0.05, rateType: 'fixed' },
        { id: '2', name: 'MBS Pool', category: 'asset', subcategory: 'mbs', balance: 50, duration: 7, rate: 0.04, rateType: 'fixed' },
        { id: '3', name: 'Commercial Loan', category: 'asset', subcategory: 'commercial', balance: 80, duration: 3, rate: 0.06, rateType: 'fixed' },
        { id: '4', name: 'Variable Rate CD', category: 'liability', subcategory: 'deposits', balance: 120, duration: 2, rate: 0.02, rateType: 'variable' },
        { id: '5', name: 'Fixed Deposits', category: 'liability', subcategory: 'deposits', balance: 60, duration: 1, rate: 0.015, rateType: 'fixed' },
      ];
      const mockPrisma = {
    ...
    it('classifies prepayable and callable instruments correctly', async () => {
      ...
      const mortgage = result.instruments.find(i => i.instrumentName === 'Residential Mortgages');
      const mbs = result.instruments.find(i => i.instrumentName === 'MBS Pool');
      const commercial = result.instruments.find(i => i.instrumentName === 'Commercial Loan');
    ...
    it('effective duration is less than modified duration for optioned instruments', async () => {
      const result = await serviceWithItems.analyzePortfolio('inst-1');
      const mortgage = result.instruments.find(i => i.instrumentName === 'Residential Mortgages');
      expect(mortgage!.effectiveDuration).toBeLessThan(mortgage!.modifiedDuration);
    });
  ...
  describe('classifyOptionType via variable rateType', () => {
    it('classifies variable-rate instruments as indexed', async () => {
      const items = [
        { id: '1', name: 'SOFR Loan', category: 'asset', subcategory: 'commercial', balance: 50, duration: 3, rate: 0.04, rateType: 'variable' },
      ];
      const mockPrisma = {
        balanceSheetItem: { findMany: jest.fn().mockResolvedValue(items) },
      } as any;
