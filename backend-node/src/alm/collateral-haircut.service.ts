...
export class CollateralHaircutService {
  calculate(
    ...
    const enriched = assets.map((a) => {
      ...
      return {
        ...a,
        haircut,
        pledgeableValue: +(a.marketValue * (1 - haircut / 100)).toFixed(0),
      };
    });
  ...
  private getHaircut(type: string, rating?: string, maturity?: number): number {
    ...
    if (t.includes('treasury') || t.includes('government'))
      return maturity && maturity > 10 ? 4 : 2;
    if (t.includes('agency') || t.includes('mbs'))
      return maturity && maturity > 10 ? 8 : 5;
    if (t.includes('municipal')) return rating === 'AAA' ? 5 : 10;
    if (t.includes('corporate'))
      return rating === 'AAA' ? 8 : rating === 'AA' ? 12 : 20;
