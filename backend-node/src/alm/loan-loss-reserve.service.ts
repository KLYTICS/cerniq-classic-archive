...
export class LoanLossReserveService {
  analyze(params: {
    ...
    const delinquencyRate =
      ((delinquent30 + delinquent60 + delinquent90) / totalLoans) * 100;
    const nplRatio = (nonPerforming / totalLoans) * 100;

    // Q-factors (qualitative adjustments)
    const qFactors = [
      {
        factor: 'Lending policies',
        factorEs: 'Politicas de prestamo',
        adjustment: delinquencyRate > 3 ? 0.15 : 0,
        reason: `Delinquency rate: ${delinquencyRate.toFixed(1)}%`,
      },
      {
        factor: 'Economic conditions',
        factorEs: 'Condiciones economicas',
        adjustment: 0.1,
        reason: 'PR economy: moderate growth with hurricane risk',
      },
      {
        factor: 'Portfolio composition',
        factorEs: 'Composicion del portafolio',
        adjustment: nplRatio > 2 ? 0.2 : 0.05,
        reason: `NPL ratio: ${nplRatio.toFixed(1)}%`,
      },
      {
        factor: 'Credit concentration',
        factorEs: 'Concentracion crediticia',
        adjustment: 0.08,
        reason: 'Sector concentration within PR cooperativa market',
      },
      {
        factor: 'Loss trend',
        factorEs: 'Tendencia de perdidas',
        adjustment: historicalLossRate > 1 ? 0.12 : 0.03,
        reason: `Historical loss rate: ${historicalLossRate.toFixed(2)}%`,
      },
    ];
    ...
    const adjustedReserve =
      baseReserve * (1 + totalQFactor) + (ceclEstimate ?? baseReserve * 0.3);
    const gap = adjustedReserve - currentALLL;

    const adequacy =
      coverageRatio >= peerCoverageRatio
        ? 'adequate'
        : coverageRatio >= peerCoverageRatio * 0.75
          ? 'marginal'
          : 'deficient';
