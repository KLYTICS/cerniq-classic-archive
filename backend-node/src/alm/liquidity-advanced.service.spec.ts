...
describe('LiquidityAdvancedService', () => {
  ...
  describe('calculateLCR', () => {
    it('computes HQLA from cash + government securities + corp bonds (haircut)', () => {
      const items = [
        { category: 'asset', subcategory: 'cash', balance: 50_000, name: 'Cash' },
        { category: 'asset', subcategory: 'government_securities', balance: 100_000, name: 'Gov' },
        { category: 'asset', subcategory: 'corporate_bonds', balance: 80_000, name: 'Corp' },
        { category: 'liability', subcategory: 'deposits', balance: 200_000, name: 'Dep', maturityYears: 0.5 },
      ];
    ...
    it('classifies LCR status correctly', () => {
      ...
      const compliant = [
        { category: 'asset', subcategory: 'cash', balance: 100_000, name: 'Cash' },
        { category: 'liability', subcategory: 'deposits', balance: 100_000, name: 'Dep', maturityYears: 0.5 },
      ];
      expect(service.calculateLCR(compliant).status).toBe('compliant');

      // Breach: very low HQLA
      const breach = [
        { category: 'asset', subcategory: 'loans', balance: 100_000, name: 'Loans' },
        { category: 'liability', subcategory: 'deposits', balance: 500_000, name: 'Dep', maturityYears: 0.5 },
      ];
    ...
    it('applies Level 2 cap (Basel III: Level 2 <= 2/3 of Level 1)', () => {
      const items = [
        { category: 'asset', subcategory: 'cash', balance: 30_000, name: 'Cash' },
        { category: 'asset', subcategory: 'corporate_bonds', balance: 200_000, name: 'Corp' },
        { category: 'liability', subcategory: 'deposits', balance: 100_000, name: 'Dep', maturityYears: 0.5 },
      ];
      const result = service.calculateLCR(items);
    ...
    it('filters short-term liabilities (maturity <= 1yr) for outflows', () => {
      const items = [
        { category: 'asset', subcategory: 'cash', balance: 100_000, name: 'Cash' },
        { category: 'liability', subcategory: 'deposits', balance: 200_000, name: 'Short', maturityYears: 0.5 },
        { category: 'liability', subcategory: 'bonds', balance: 300_000, name: 'Long', maturityYears: 5 },
      ];
      const result = service.calculateLCR(items);
  ...
  describe('NSFR additional factors', () => {
    it('assigns ASF 1.0 for time deposits > 1yr', () => {
      const items = [
        { category: 'liability', subcategory: 'time_deposit', balance: 100_000, name: 'CD', duration: 2 },
        { category: 'asset', subcategory: 'cash', balance: 50_000, name: 'Cash' },
      ];
      const result = service.calculateNSFR(items);
    ...
    it('assigns ASF 0.5 for time deposits <= 1yr', () => {
      const items = [
        { category: 'liability', subcategory: 'cd_short', balance: 100_000, name: 'Short CD', duration: 0.5 },
        { category: 'asset', subcategory: 'cash', balance: 50_000, name: 'Cash' },
      ];
      const result = service.calculateNSFR(items);
    ...
    it('assigns ASF 0.0 for short-term borrowings', () => {
      const items = [
        { category: 'liability', subcategory: 'fhlb_borrowing', balance: 100_000, name: 'FHLB', duration: 0.5 },
        { category: 'asset', subcategory: 'cash', balance: 50_000, name: 'Cash' },
      ];
      const result = service.calculateNSFR(items);
    ...
    it('assigns ASF 1.0 for long-term borrowings', () => {
      const items = [
        { category: 'liability', subcategory: 'fhlb_borrowing', balance: 100_000, name: 'FHLB', duration: 3 },
        { category: 'asset', subcategory: 'cash', balance: 50_000, name: 'Cash' },
      ];
      const result = service.calculateNSFR(items);
      const fhlb = result.asfBreakdown.find(a => a.category === 'FHLB');
    ...
    it('assigns RSF 0.15 for agency/MBS securities', () => {
      const items = [
        { category: 'asset', subcategory: 'agency_mbs', balance: 100_000, name: 'MBS' },
      ];
      const result = service.calculateNSFR(items);
    ...
    it('assigns RSF 0.65 for residential mortgages', () => {
      const items = [
        { category: 'asset', subcategory: 'residential_mortgage', balance: 100_000, name: 'Mortgage' },
      ];
      const result = service.calculateNSFR(items);
    ...
    it('assigns RSF 0.85 for commercial RE', () => {
      const items = [
        { category: 'asset', subcategory: 'commercial_re', balance: 100_000, name: 'CRE' },
      ];
      const result = service.calculateNSFR(items);
    ...
    it('assigns RSF 0.5 for short-term consumer loans', () => {
      const items = [
        { category: 'asset', subcategory: 'consumer_loan', balance: 100_000, name: 'Consumer', duration: 0.5 },
      ];
      const result = service.calculateNSFR(items);
    ...
    it('assigns RSF 1.0 for unknown asset types', () => {
      const items = [
        { category: 'asset', subcategory: 'other_exotic', balance: 100_000, name: 'Exotic' },
      ];
      const result = service.calculateNSFR(items);
    ...
    it('computes warning NSFR for 90-100 range', () => {
      const items = [
        { category: 'liability', subcategory: 'other', balance: 950_000, name: 'Liab' },
        { category: 'asset', subcategory: 'other', balance: 1_000_000, name: 'Asset' },
      ];
      const result = service.calculateNSFR(items);
  ...
  describe('deposit flight survival horizon', () => {
    it('detects survival horizon when HQLA depletes', () => {
      const tiers = [
        { tierName: 'Uninsured', balance: 1_000_000, insuredPct: 0, flightRate: 0.2 },
      ];
    ...
    it('survival horizon is 12 when HQLA never depletes', () => {
      const tiers = [
        { tierName: 'Core', balance: 100_000, insuredPct: 0.9, flightRate: 0.01 },
      ];
    ...
    it('expectedLoss is 30% of 6-month loss', () => {
      const tiers = [
        { tierName: 'Core', balance: 500_000, insuredPct: 0.8, flightRate: 0.03 },
      ];
      const result = service.simulateDepositFlight(tiers, 100_000);
      const totalMonth6Loss = result.tiers.reduce((s, t) => s + t.month6Loss, 0);
  ...
  describe('deposit flight default HQLA', () => {
    it('uses 15% of tier balances as HQLA when totalHQLA is 0', () => {
      const tiers = [
        { tierName: 'Core', balance: 1_000_000, insuredPct: 0.9, flightRate: 0.02 },
      ];
  ...
  describe('getAdvancedLiquidity', () => {
    it('returns combined LCR, NSFR, and deposit flight', async () => {
      mockPrisma.balanceSheetItem.findMany.mockResolvedValue([
        { category: 'asset', subcategory: 'cash', balance: 100_000, name: 'Cash' },
        { category: 'liability', subcategory: 'savings', balance: 200_000, name: 'Savings' },
      ]);
    ...
    it('uses default tiers when no deposit tiers exist', async () => {
      mockPrisma.balanceSheetItem.findMany.mockResolvedValue([
        { category: 'asset', subcategory: 'cash', balance: 50_000, name: 'Cash' },
        { category: 'liability', subcategory: 'deposits', balance: 200_000, name: 'Deposits' },
      ]);
      ...
      expect(result.depositFlight.tiers.map(t => t.tierName)).toContain('Insured Core');
    });

    it('uses actual deposit tiers when they exist', async () => {
      mockPrisma.balanceSheetItem.findMany.mockResolvedValue([
        { category: 'asset', subcategory: 'cash', balance: 50_000, name: 'Cash' },
        { category: 'liability', subcategory: 'deposits', balance: 200_000, name: 'Dep' },
      ]);
      mockPrisma.liquidityPosition.findFirst.mockResolvedValue(null);
      mockPrisma.depositTier.findMany.mockResolvedValue([
        { tierName: 'Custom Tier', balance: 200_000, insuredPct: 0.7, flightRate: 0.04 },
      ]);
