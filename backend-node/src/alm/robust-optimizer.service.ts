...
export interface RobustOptResult {
  ...
  bestAllocation: Array<{ subcategory: string; currentBalance: number; suggestedDelta: number }>;
...
export class RobustOptimizerService {
  ...
  async optimize(institutionId: string, aggressiveness: 'conservative' | 'moderate' | 'aggressive' = 'moderate'): Promise<RobustOptResult> {
    ...
    const maxMovePct = { conservative: 0.03, moderate: 0.06, aggressive: 0.10 }[aggressiveness];
    ...
    for (const pert of perturbations) {
      ...
      for (const scenario of scenarios) {
        ...
        const baseNII = adjustedItems.filter(i => i.category === 'asset').reduce((s, i) => s + i.balance * i.rate, 0)
          - adjustedItems.filter(i => i.category === 'liability').reduce((s, i) => s + i.balance * i.rate, 0);
        // Simplified: adjust NII by rate path average
        const avgRateShock = scenario.ratePathBps.reduce((a, b) => a + b, 0) / scenario.ratePathBps.length / 10000;
        const stressedNII = baseNII * (1 + avgRateShock * 3) * (1 + scenario.gdpDelta);
    ...
    const bestPerScenario = scenarios.map((_, s) => Math.max(...perfMatrix.map(row => row[s])));

    // Max regret per perturbation
    const maxRegrets = perfMatrix.map(row => Math.max(...row.map((nii, s) => bestPerScenario[s] - nii)));
    ...
    return {
      strategy: 'minimax-regret',
      bestAllocation: perturbations[bestIdx].map(p => ({
  ...
  private generatePerturbations(items: any[], maxMove: number): Array<Array<{ subcategory: string; currentBalance: number; delta: number }>> {
    const bySub = new Map<string, number>();
    for (const item of items) {
      bySub.set(item.subcategory, (bySub.get(item.subcategory) ?? 0) + item.balance);
    }

    const subs = Array.from(bySub.entries());
    const perturbations: Array<Array<{ subcategory: string; currentBalance: number; delta: number }>> = [];

    // Status quo
    perturbations.push(subs.map(([sub, bal]) => ({ subcategory: sub, currentBalance: bal, delta: 0 })));
    ...
    for (let i = 0; i < Math.min(subs.length, 5); i++) {
      for (let j = i + 1; j < Math.min(subs.length, 5); j++) {
        ...
        if (move < 1) continue;
        perturbations.push(subs.map(([sub, bal]) => ({
          subcategory: sub, currentBalance: bal,
  ...
  private applyPerturbation(items: any[], perturbation: Array<{ subcategory: string; delta: number }>): any[] {
    const deltaMap = new Map(perturbation.map(p => [p.subcategory, p.delta]));
    return items.map(i => ({
      ...i,
      balance: i.balance + (deltaMap.get(i.subcategory) ?? 0) / items.filter(x => x.subcategory === i.subcategory).length,
    }));
  }
}
