...
export class MarginCompressionService {
  analyze(params: {
    ...
    annualCompressionBps: number;
    projectedNIM12M: number;
    assetYieldPressure: number;
    fundingCostPressure: number;
    ...
    const trend =
      annualCompressionBps < -5
        ? 'compressing'
        : annualCompressionBps > 5
          ? 'expanding'
          : 'stable';
    const assetYieldPressure = +(competitorRates.loanRate - assetYield).toFixed(
      4,
    );
    const fundingCostPressure = +(
      fundingCost - competitorRates.savingsRate
    ).toFixed(4);

    return {
      currentNIM,
      trend,
      annualCompressionBps,
      projectedNIM12M,
      assetYieldPressure,
      fundingCostPressure,
