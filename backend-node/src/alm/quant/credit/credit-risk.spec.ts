...
describe('normalInverse', () => {
  ...
  it('round-trip: N(N_inv(p)) = p', () => {
    for (const p of [0.01, 0.05, 0.10, 0.25, 0.50, 0.75, 0.90, 0.95, 0.99]) {
...
describe('computeEffectiveLGD', () => {
  it('hurricane adjustment increases LGD for residential mortgage by exactly 10%', () => {
    ...
    expect(hurricane - base).toBeCloseTo(0.10, 4);
  });
...
describe('estimatePD', () => {
  it('PD decreases with higher DSCR (better coverage)', () => {
    const pdLow = estimatePD('RESIDENTIAL_MORTGAGE', { dscr: 0.8, ltv: 0.7, delinquencyRate: 0.03 });
    const pdHigh = estimatePD('RESIDENTIAL_MORTGAGE', { dscr: 2.0, ltv: 0.7, delinquencyRate: 0.03 });
    expect(pdHigh).toBeLessThan(pdLow);
  });

  it('PD increases with higher LTV', () => {
    const pdLow = estimatePD('RESIDENTIAL_MORTGAGE', { dscr: 1.2, ltv: 0.5, delinquencyRate: 0.03 });
    const pdHigh = estimatePD('RESIDENTIAL_MORTGAGE', { dscr: 1.2, ltv: 0.95, delinquencyRate: 0.03 });
    expect(pdHigh).toBeGreaterThan(pdLow);
  });

  it('PD increases with higher delinquency rate', () => {
    const pdLow = estimatePD('RESIDENTIAL_MORTGAGE', { dscr: 1.2, ltv: 0.7, delinquencyRate: 0.01 });
    const pdHigh = estimatePD('RESIDENTIAL_MORTGAGE', { dscr: 1.2, ltv: 0.7, delinquencyRate: 0.10 });
    expect(pdHigh).toBeGreaterThan(pdLow);
  });

  it('PD is clamped between 0.001 and 0.999', () => {
    // Extreme inputs that would push PD to limits
    const pdFloor = estimatePD('RESIDENTIAL_MORTGAGE', { dscr: 10, ltv: 0, delinquencyRate: 0 });
    const pdCap = estimatePD('CONSUMER_UNSECURED', { dscr: 0, ltv: 1, delinquencyRate: 0.5 });
    ...
    categories: [
      {
        ...
        financialRatios: { dscr: 1.3, ltv: 0.70, delinquencyRate: 0.03 },
      },
      {
      ...
      },
      {
        ...
        financialRatios: { dscr: 1.5, ltv: 0.60, delinquencyRate: 0.02 },
      },
      {
