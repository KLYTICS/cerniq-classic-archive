...
const PEER_RATES: Record<string, number> = {
  consumer_loans: 0.072, auto_loans: 0.065, residential_mortgage: 0.055,
  commercial_re: 0.058, commercial_loans: 0.068, credit_cards: 0.145,
...
export class NIMOptimizerService {
  ...
  async optimize(institutionId: string): Promise<NIMOptimizerResult> {
    ...
    const currentNII = assets.reduce((s, i) => s + i.balance * i.rate, 0) - liabilities.reduce((s, i) => s + i.balance * i.rate, 0);
    ...
    const bySub = new Map<string, { balance: number; weightedRate: number; category: string }>();
    for (const item of items) {
      const key = item.subcategory;
      if (!bySub.has(key)) bySub.set(key, { balance: 0, weightedRate: 0, category: item.category });
    ...
    for (const [sub, entry] of bySub) {
      const avgRate = entry.balance > 0 ? entry.weightedRate / entry.balance : 0;
      ...
      if (isAsset && gapBps < -25) {
        ...
        recommendations.push({
          product: sub.replace(/_/g, ' '), category: 'asset',
          currentRate: avgRate, peerMedianRate: peerRate, suggestedRate,
          rateDeltaBps: Math.round((suggestedRate - avgRate) * 10000),
          direction: 'increase', niiImpact: Math.round(niiGain * 100) / 100,
        ...
        recommendations.push({
          product: sub.replace(/_/g, ' '), category: 'liability',
          currentRate: avgRate, peerMedianRate: peerRate, suggestedRate,
          rateDeltaBps: Math.round((suggestedRate - avgRate) * 10000),
          direction: 'decrease', niiImpact: Math.round(niiGain * 100) / 100,
          volumeImpact: 'Monitor deposit outflows — use beta library for sensitivity estimate',
    ...
    recommendations.sort((a, b) => Math.abs(b.niiImpact) - Math.abs(a.niiImpact));
    const projectedNIM = totalAssets > 0 ? ((currentNII + totalNIIGain) / totalAssets) * 100 : currentNIM;
  ...
  private getDemoResult(): NIMOptimizerResult {
    return {
      currentNIM: 3.42, projectedNIM: 3.68, nimGainBps: 26, totalNIIGain: 1.16,
      recommendations: [
        { product: 'time deposits', category: 'liability', currentRate: 0.045, peerMedianRate: 0.040, suggestedRate: 0.040, rateDeltaBps: -50, direction: 'decrease', niiImpact: 0.38, volumeImpact: 'Low — CDs are rate-insensitive in short term', rationale: 'CD rates 50bps above peer median.', rationaleEs: 'Tasas CD 50bps por encima de mediana.' },
        { product: 'consumer loans', category: 'asset', currentRate: 0.068, peerMedianRate: 0.072, suggestedRate: 0.072, rateDeltaBps: 40, direction: 'increase', niiImpact: 0.34, volumeImpact: 'Minimal — still competitive', rationale: 'Consumer loan rates 40bps below market.', rationaleEs: 'Tasas consumo 40bps bajo mercado.' },
      ],
    };
  }
}
