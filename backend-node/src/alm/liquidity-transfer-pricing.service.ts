...
const INTERNAL_FUNDING_CURVE: Array<{ bucket: string; minYears: number; maxYears: number; fundingCost: number; riskFreeRate: number; liquidityPremium: number }> = [
  { bucket: '0-3M', minYears: 0, maxYears: 0.25, fundingCost: 0.050, riskFreeRate: 0.048, liquidityPremium: 0.002 },
  { bucket: '3-12M', minYears: 0.25, maxYears: 1, fundingCost: 0.047, riskFreeRate: 0.044, liquidityPremium: 0.003 },
  { bucket: '1-3Y', minYears: 1, maxYears: 3, fundingCost: 0.045, riskFreeRate: 0.041, liquidityPremium: 0.004 },
  { bucket: '3-5Y', minYears: 3, maxYears: 5, fundingCost: 0.046, riskFreeRate: 0.0405, liquidityPremium: 0.0055 },
  { bucket: '5-10Y', minYears: 5, maxYears: 10, fundingCost: 0.048, riskFreeRate: 0.042, liquidityPremium: 0.006 },
  { bucket: '>10Y', minYears: 10, maxYears: 999, fundingCost: 0.052, riskFreeRate: 0.046, liquidityPremium: 0.006 },
];
...
export class LiquidityTransferPricingService {
  ...
  async computeLTP(institutionId: string): Promise<LTPResult> {
    ...
    const bySub = new Map<string, { balance: number; rate: number; category: string; duration: number }>();
    for (const item of items) {
      if (!bySub.has(item.subcategory)) {
        bySub.set(item.subcategory, { balance: 0, rate: 0, category: item.category, duration: 0 });
      }
    ...
    for (const [sub, entry] of bySub) {
      ...
      const avgDuration = entry.balance > 0 ? entry.duration / entry.balance : 1;
      const isAsset = entry.category === 'asset';

      const bucket = INTERNAL_FUNDING_CURVE.find(b => avgDuration >= b.minYears && avgDuration < b.maxYears)
    ...
    segments.sort((a, b) => Math.abs(b.liquidityCharge) - Math.abs(a.liquidityCharge));
    ...
    return {
      ...
      topConsumers: segments.filter(s => s.liquidityCharge > 0).slice(0, 3).map(s => s.segment),
      topProviders: segments.filter(s => s.liquidityCharge < 0).slice(0, 3).map(s => s.segment),
    };
  }
