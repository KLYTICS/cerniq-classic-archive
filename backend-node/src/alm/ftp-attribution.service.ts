...
const CREDIT_SPREAD_BY_TYPE: Record<string, number> = {
  cash: 0, securities: 0.002, consumer_loans: 0.018, auto_loans: 0.012,
  residential_mortgage: 0.004, commercial_re: 0.008, commercial_loans: 0.015,
  credit_cards: 0.035, other_assets: 0.005,
};
...
export class FTPAttributionService {
  ...
  async getFullAttribution(institutionId: string): Promise<FTPAttributionResult> {
    const ftpAnalysis = await this.ftp.getFTPAnalysis(institutionId);

    const decompositions: SpreadDecomposition[] = ftpAnalysis.instruments.map(inst => {
      const sub = inst.subcategory.toLowerCase();
      const isAsset = inst.category === 'asset';

      const ftpNet = isAsset ? inst.actualRate - inst.ftpRate : inst.ftpRate - inst.actualRate;
      const creditSpread = isAsset ? (CREDIT_SPREAD_BY_TYPE[sub] ?? 0.005) : 0;
      ...
      const economicProfit = inst.balance * (ftpNet - creditSpread - optionCost - liquidityPremium - opCost);
    ...
    const rarocMap = new Map<string, { balance: number; profit: number; category: string }>();
    for (const d of decompositions) {
      const key = `${d.category}:${d.subcategory}`;
      if (!rarocMap.has(key)) rarocMap.set(key, { balance: 0, profit: 0, category: d.category });
    ...
    const rarocRanking: RarocRanking[] = Array.from(rarocMap.entries())
      .map(([key, { balance, profit, category }]) => {
        ...
        return {
          ...
          verdict: raroc > 0.12 ? 'ACCRETIVE' as const : raroc >= 0.05 ? 'NEUTRAL' as const : 'DESTRUCTIVE' as const,
        };
      })
      .sort((a, b) => b.raroc - a.raroc);

    const totalProfit = decompositions.reduce((s, d) => s + d.economicProfit, 0);
    ...
    return {
      ...
      summary: {
        ...
        totalFTPNet: decompositions.reduce((s, d) => s + d.balance * d.ftpNet, 0),
        totalCreditCost: decompositions.reduce((s, d) => s + d.balance * d.creditSpread, 0),
        totalOptionCost: decompositions.reduce((s, d) => s + d.balance * d.optionCost, 0),
        totalLiquidityCost: decompositions.reduce((s, d) => s + d.balance * d.liquidityPremium, 0),
        totalOpCost: decompositions.reduce((s, d) => s + d.balance * d.opCost, 0),
        totalEconomicProfit: Math.round(totalProfit * 100) / 100,
        portfolioRaroc: totalBalance > 0 ? Math.round(totalProfit / (totalBalance * 0.08) * 10000) / 10000 : 0,
      },
    };
  }
}
