...
describe('RecoveryRateService', () => {
  ...
  it('estimates LGD by collateral type for a portfolio', () => {
    const result = service.estimate([
      { type: 'Mortgage', typeEs: 'Hipoteca', balance: 200, collateralValue: 250, collateralType: 'real estate', seniorityRank: 1 },
      { type: 'Auto', typeEs: 'Auto', balance: 50, collateralValue: 40, collateralType: 'auto', seniorityRank: 1 },
      { type: 'Unsecured', typeEs: 'No garantizado', balance: 30, collateralValue: 0, collateralType: 'unsecured', seniorityRank: 1 },
    ]);
  ...
  it('real estate gets highest base recovery rate (0.65)', () => {
    const result = service.estimate([
      { type: 'RE', typeEs: 'RE', balance: 100, collateralValue: 120, collateralType: 'real estate', seniorityRank: 1 },
    ]);
  ...
  it('seniority rank reduces recovery for junior tranches', () => {
    const senior = service.estimate([
      { type: 'A', typeEs: 'A', balance: 100, collateralValue: 150, collateralType: 'real estate', seniorityRank: 1 },
    ]);
    const junior = service.estimate([
      { type: 'B', typeEs: 'B', balance: 100, collateralValue: 150, collateralType: 'real estate', seniorityRank: 3 },
    ]);

    expect(senior.loans[0].estimatedRecovery).toBeGreaterThan(junior.loans[0].estimatedRecovery);
  });
  ...
  it('unsecured loans have lowest base recovery (0.25)', () => {
    const result = service.estimate([
      { type: 'Personal', typeEs: 'Personal', balance: 100, collateralValue: 100, collateralType: 'unsecured', seniorityRank: 1 },
    ]);
