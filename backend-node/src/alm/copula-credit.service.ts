...
export class CopulaCreditService {
  ...
  async simulateWithCopula(
    ...
    const pds = segments.map(s => Math.min(0.3, s.historicalLossRate * 2));
    const lgds = segments.map(s => s.lgd);
    const eads = segments.map(s => s.balance);
    ...
    const es99 = losses.slice(Math.floor(paths * 0.99)).reduce((s, v) => s + v, 0) / (paths * 0.01);

    // Tail dependence (t-copula)
    const tailDep = copulaType === 't-copula'
      ? 2 * this.tCDF(-Math.sqrt((nuDof + 1) * (1 - rho) / (1 + rho)), nuDof + 1)
      : 0;

    // Joint default probability
    const jointDefault = losses.filter(l => l > eads.reduce((s, e) => s + e, 0) * 0.5).length / paths;

    // t-copula premium: how much more risk than Gaussian
    let tPremium = 0;
    if (copulaType === 't-copula') {
      // Run Gaussian for comparison
      const gaussResult = await this.simulateWithCopula(institutionId, 'gaussian', 5, Math.min(paths, 5000));
    ...
    return {
      ...
      segments: segments.map(s => s.segmentName),
    };
  }
  ...
  private chiSquaredSample(rng: () => number, df: number): number {
    let sum = 0;
    for (let i = 0; i < df; i++) { const z = this.normalRandom(rng); sum += z * z; }
    return sum;
  }
  ...
  private tCDF(x: number, nu: number): number {
    ...
    return 0.5 + 0.5 * Math.sign(t) * (1 - Math.exp(-0.5 * t * t * (1 + 0.25 * t * t / nu)));
  }
  ...
  private seededRNG(seed: number): () => number { let s = seed; return () => { s = (s * 1664525 + 1013904223) & 0xFFFFFFFF; return (s >>> 0) / 0xFFFFFFFF; }; }
  private normalRandom(rng: () => number): number { const u1 = rng() || 1e-10, u2 = rng(); return Math.sqrt(-2 * Math.log(u1)) * Math.cos(2 * Math.PI * u2); }

  private getDemoResult(method: string): CopulaResult {
    return { method: method as any, var99: 22.5, var999: 35.8, es99: 28.4, tailDependence: method === 't-copula' ? 0.12 : 0, tCopulaPremium: method === 't-copula' ? 4.2 : 0, jointDefaultProbability: 0.0008, correlationMatrix: [[1, 0.15], [0.15, 1]], segments: ['Consumer', 'Commercial'] };
  }
}
