...
export interface VasicekFongResult {
  ...
  interpolatedCurve: Array<{ tenor: number; rate: number; forwardRate: number }>;
...
export class VasicekFongService {
  ...
  fitCurve(observedRates: Array<{ tenor: number; rate: number }>): VasicekFongResult {
    ...
    for (let i = 0; i < interpolatedCurve.length - 2; i++) {
      const d2 = interpolatedCurve[i + 2].rate - 2 * interpolatedCurve[i + 1].rate + interpolatedCurve[i].rate;
      smoothness += d2 * d2;
    }
    ...
    for (const obs of sorted) {
      const fitted = interpolatedCurve.reduce((best, p) =>
        Math.abs(p.tenor - obs.tenor) < Math.abs(best.tenor - obs.tenor) ? p : best
      );
  ...
  private naturalCubicSpline(x: number[], y: number[]): { a: number[]; b: number[]; c: number[]; d: number[] } {
    const n = x.length - 1;
  ...
  private evaluateSpline(spline: { a: number[]; b: number[]; c: number[]; d: number[] }, x: number[], t: number): number {
    let i = x.length - 2;
    for (let j = 0; j < x.length - 1; j++) {
      if (t <= x[j + 1]) { i = j; break; }
    }
    const dx = t - x[i];
