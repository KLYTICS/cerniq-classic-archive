...
export class VintageAnalysisService {
  analyze(vintages: Array<{ year: number; originalBalance: number; currentBalance: number; cumulativeLoss: number; delinquent: number }>): {
    vintages: Array<{ year: number; originalBalance: number; currentBalance: number; lossRate: number; delinquencyRate: number; seasoning: number }>;
    worstVintage: number; bestVintage: number; avgLossRate: number;
    interpretation: string; interpretationEs: string;
  } {
    const currentYear = new Date().getFullYear();
    const enriched = vintages.map(v => ({
      ...v, lossRate: +(v.cumulativeLoss / v.originalBalance * 100).toFixed(2),
    ...
    const avg = +(enriched.reduce((s, v) => s + v.lossRate, 0) / enriched.length).toFixed(2);

    return {
      vintages: enriched, worstVintage: worst.year, bestVintage: best.year, avgLossRate: avg,
