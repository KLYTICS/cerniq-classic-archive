...
export class HRPService {
  ...
  async computeHRP(institutionId: string): Promise<HRPResult> {
    ...
    const bySub = new Map<string, { balance: number; rate: number; vol: number }>();
    for (const item of items) {
      if (!bySub.has(item.subcategory)) bySub.set(item.subcategory, { balance: 0, rate: 0, vol: 0 });
    ...
    const rho = 0.3;
    const corrMatrix = vols.map((_, i) => vols.map((_, j) => i === j ? 1.0 : rho));
    const covMatrix = vols.map((vi, i) => vols.map((vj, j) => vi * vj * corrMatrix[i][j]));

    // Step 1: Distance matrix from correlation
    const dist = corrMatrix.map(row => row.map(r => Math.sqrt(0.5 * (1 - r))));
    ...
    // Portfolio metrics
    const portVar = normWeights.reduce((s, wi, i) =>
      s + wi * normWeights.reduce((s2, wj, j) => s2 + wj * covMatrix[i][j], 0), 0);
  ...
  private hierarchicalClustering(dist: number[][], n: number): { order: number[]; dendrogram: HRPResult['dendrogramLevels'] } {
    ...
    while (active.size > 1) {
      ...
      for (let a = 0; a < ids.length; a++) {
        for (let b = a + 1; b < ids.length; b++) {
          ...
          let d = Infinity;
          for (const i of ci) for (const j of cj) if (dist[i]?.[j] < d) d = dist[i][j];
          if (d < minDist) { minDist = d; bestI = ids[a]; bestJ = ids[b]; }
        }
      }
      ...
      active.delete(bestJ);
      active.add(nextId);
      dendrogram.push({ left: bestI, right: bestJ, distance: +minDist.toFixed(4) });
      nextId++;
    }

    // Seriation order from dendrogram
    const order = clusters.get([...active][0]) ?? Array.from({ length: n }, (_, i) => i);
    return { order, dendrogram };
  }

  private recursiveBisection(weights: number[], items: number[], cov: number[][]): void {
    ...
    left.forEach(i => { weights[i] *= alphaL; });
    right.forEach(i => { weights[i] *= (1 - alphaL); });
  ...
  private clusterVariance(items: number[], cov: number[][]): number {
    ...
    return w.reduce((s, wi, i) => s + wi * w.reduce((s2, wj, j) => s2 + wj * (cov[items[i]]?.[items[j]] ?? 0), 0), 0);
  }
  ...
  private getDemoResult(): HRPResult {
    return {
      ...
      portfolioVol: 0.038, diversificationRatio: 1.32,
      clusterOrder: [0, 1, 3, 2, 4, 5],
      dendrogramLevels: [{ left: 0, right: 1, distance: 0.18 }, { left: 4, right: 5, distance: 0.22 }],
    };
  }
}
