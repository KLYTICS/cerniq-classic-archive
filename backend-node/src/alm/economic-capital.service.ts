...
export class EconomicCapitalService {
  calculate(params: {
    ...
    surplus: number;
    components: Array<{ risk: string; riskEs: string; standalone: number; diversified: number; pct: number }>;
    interpretation: string;
    interpretationEs: string;
  } {
    const { creditRiskCapital, marketRiskCapital, operationalRiskCapital, concentrationAddOn, totalAssets, regulatoryCapital } = params;
    ...
    const standalone = [creditRiskCapital, marketRiskCapital, operationalRiskCapital];
    const undiversified = standalone.reduce((s, c) => s + c, 0) + concentrationAddOn;
    ...
    const components = [
      { risk: 'Credit Risk', riskEs: 'Riesgo Crediticio', standalone: creditRiskCapital, diversified: creditRiskCapital * diversified / undiversified, pct: creditRiskCapital / undiversified * 100 },
      { risk: 'Market Risk', riskEs: 'Riesgo de Mercado', standalone: marketRiskCapital, diversified: marketRiskCapital * diversified / undiversified, pct: marketRiskCapital / undiversified * 100 },
      { risk: 'Operational Risk', riskEs: 'Riesgo Operacional', standalone: operationalRiskCapital, diversified: operationalRiskCapital * diversified / undiversified, pct: operationalRiskCapital / undiversified * 100 },
      { risk: 'Concentration', riskEs: 'Concentracion', standalone: concentrationAddOn, diversified: concentrationAddOn, pct: concentrationAddOn / undiversified * 100 },
    ].map(c => ({ ...c, standalone: +c.standalone.toFixed(0), diversified: +c.diversified.toFixed(0), pct: +c.pct.toFixed(1) }));
