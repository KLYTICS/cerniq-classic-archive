...
export class DepositBetaLibraryService {
  ...
  async getBenchmark(institutionId: string): Promise<BetaLibraryResult> {
    ...
    const sizeTier = totalAssets < 50 ? 'small' : totalAssets < 300 ? 'medium' : 'large';
    ...
    const benchmarks: BetaBenchmark[] = Object.keys(this.library.betas).map(subcategory => {
      const peerData = (this.library.betas as any)[subcategory];
      const instBeta = institutionBetas.get(subcategory) ?? peerData[sizeTier];
      ...
      return {
        ...
        nationalMedian: (this.library.nationalComparison as any)[subcategory] ?? 0.5,
    ...
    return {
      ...
      insightEs: 'Los betas de depósito de cooperativas PR son sistemáticamente 25-40% más bajos que los promedios nacionales de bancos asegurados FDIC, impulsados por la lealtad del socio, alternativas limitadas y la psicología de propiedad cooperativa.',
    };
  }
  ...
  private normalizeSubcategory(sub: string): string {
    ...
    if (s.includes('demand') || s.includes('checking')) return 'demand_deposits';
    ...
    if (s.includes('time') || s.includes('cd') || s.includes('plazo') || s.includes('certificate')) return 'time_deposits';
    return s;
  }

  private getDefaultBeta(sub: string): number {
    const defaults: Record<string, number> = {
      demand_deposits: 0.10, savings_deposits: 0.18, share_drafts: 0.13,
      money_market: 0.41, iras: 0.58, time_deposits: 0.79,
    };
    return defaults[sub] ?? 0.40;
  }
}
