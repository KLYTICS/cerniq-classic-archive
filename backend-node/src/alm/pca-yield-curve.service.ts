...
export class PCAYieldCurveService {
  ...
  computePCAFactors(yieldChanges: number[][]): PCAResult {
    ...
    const tenorLabels = ['3M', '6M', '1Y', '2Y', '3Y', '5Y', '7Y', '10Y', '20Y', '30Y'].slice(0, m);

    // Demean
    const means = Array(m).fill(0).map((_, j) => yieldChanges.reduce((s, r) => s + (r[j] ?? 0), 0) / n);
    ...
    return {
      factors,
      totalExplainedPct: +factors.reduce((s, f) => s + f.explainedVariancePct, 0).toFixed(1),
      tenorLabels,
    };
  }

  // Attribute portfolio DV01 to PCA factors
  attributeDV01(portfolioDV01: number[], factors: PCAFactor[]): { level: number; slope: number; curvature: number } {
    const project = (loadings: number[]) => loadings.reduce((s, l, i) => s + l * (portfolioDV01[i] ?? 0), 0);
    return {
  ...
  generateSyntheticChanges(baseRates: number[], weeks: number = 52): number[][] {
  ...
  private getDemoResult(): PCAResult {
    return {
      factors: [
        { name: 'Level', loadings: [0.31, 0.32, 0.33, 0.33, 0.33, 0.32, 0.32, 0.31, 0.30, 0.29], explainedVariancePct: 89.2, eigenvalue: 0.000245 },
        { name: 'Slope', loadings: [-0.42, -0.35, -0.25, -0.10, 0.05, 0.20, 0.30, 0.38, 0.42, 0.43], explainedVariancePct: 7.8, eigenvalue: 0.000021 },
        { name: 'Curvature', loadings: [0.35, 0.20, -0.05, -0.30, -0.40, -0.30, -0.10, 0.15, 0.40, 0.45], explainedVariancePct: 2.1, eigenvalue: 0.0000058 },
      ],
      totalExplainedPct: 99.1,
      tenorLabels: ['3M', '6M', '1Y', '2Y', '3Y', '5Y', '7Y', '10Y', '20Y', '30Y'],
    };
  }
}
