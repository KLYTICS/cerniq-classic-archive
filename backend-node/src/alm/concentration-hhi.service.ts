...
export class ConcentrationHHIService {
  calculate(segments: Array<{ name: string; nameEs: string; balance: number }>): {
    hhi: number; hhiScaled: number; classification: string; classificationEs: string;
    effectiveSegments: number; // 1/HHI — equivalent number of equal segments
    segments: Array<{ name: string; nameEs: string; balance: number; share: number; contribution: number }>;
    ...
    const enriched = segments.map(seg => {
      ...
      return { ...seg, share: +share.toFixed(4), contribution: +(share * share * 10000).toFixed(1) };
    ...
    const classification = hhiScaled < 1500 ? 'Unconcentrated' : hhiScaled < 2500 ? 'Moderately concentrated' : 'Highly concentrated';
    const classificationEs = hhiScaled < 1500 ? 'No concentrado' : hhiScaled < 2500 ? 'Moderadamente concentrado' : 'Altamente concentrado';

    return {
      hhi: +hhi.toFixed(6), hhiScaled, classification, classificationEs, effectiveSegments, segments: enriched,
