...
export class ForwardSimulationService {
  ...
  async runForwardSimulation(config: ForwardSimConfig): Promise<ForwardSimResult> {
    ...
    for (const path of paths) {
      ...
      let projectedLiabilities = [...items.filter(i => i.category === 'liability')];

      for (let q = 1; q <= totalQuarters; q++) {
        const year = Math.ceil(q / 4);
        const quarterLabel = `Q${((q - 1) % 4) + 1} ${new Date().getFullYear() + Math.ceil(q / 4) - 1}`;
        const quarterlyGrowth = (growthFactor: number) => Math.pow(1 + growthFactor, 1 / 4);
        ...
        for (const item of projectedAssets) {
          ...
          const rate = item.rate + (shockBps / 10000) * (item.rateType === 'variable' ? 1.0 : q <= 4 ? 0 : 0.3);
        ...
        const lcr = 100 + (totalAssetBal * 0.15 / (totalLiabBal * 0.08) - 1) * 50; // simplified
        ...
        quarters.push({
          ...
          projectedNSFR: Math.min(200, Math.max(50, Math.round(nsfr * 10) / 10)),
        ...
        if (q === totalQuarters) {
          ...
          projectedLiabilities = [...items.filter(i => i.category === 'liability')];
        }
      }
    }
    ...
    return {
      ...
      summary: {
        ...
        up200NIIYear3: up200Q.length > 0 ? up200Q.reduce((s, q) => s + q.projectedNII, 0) : 0,
        down100NIIYear3: down100Q.length > 0 ? down100Q.reduce((s, q) => s + q.projectedNII, 0) : 0,
  ...
  private getDefaultBeta(sub: string): number {
    ...
    if (sub.includes('time') || sub.includes('cd') || sub.includes('plazo')) return 0.80;
    return 0.50;
  }
  ...
  private getDemoResult(horizon: number, paths: string[]): ForwardSimResult {
    ...
    for (const path of paths) {
      ...
      for (let q = 1; q <= totalQ; q++) {
        ...
        quarters.push({
          ...
          ratePath: path,
          projectedNII: Math.round((baseNII * growthFactor + baseNII * rateEffect) * 100) / 100,
          projectedEVE: Math.round((52 * growthFactor - shock * 0.08 * q / totalQ) * 100) / 100,
          projectedLCR: Math.min(180, Math.max(80, 115 + shock * 0.01 - q * 0.5)),
    ...
    return {
      ...
      quarters,
      summary: {
        baseNIIYear1: 12.8, baseNIIYear3: 41.2, up200NIIYear3: 48.5,
        down100NIIYear3: 35.8, worstCaseNWR: 6.8, worstCaseLCR: 88,
      },
    };
  }
}
