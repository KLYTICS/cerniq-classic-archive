...
const LIQUIDITY_HORIZONS: Record<string, number> = {
  interest_rate_large: 10, interest_rate_small: 20, interest_rate_vol: 60,
  credit_spread_ig: 40, credit_spread_hy: 60,
  equity_large: 10, equity_small: 20, fx_major: 10, fx_other: 20,
};
...
export interface FRTBResult {
  ...
  byRiskClass: Array<{ riskClass: string; es: number; liquidityHorizon: number }>;
}
...
export class FRTBESService {
  ...
  async computeFRTBCapital(institutionId: string): Promise<FRTBResult> {
    ...
    const es975 = -tailReturns.reduce((s, r) => s + r, 0) / Math.max(tailCount, 1);
    ...
    const trafficLight = exceptions <= 4 ? 'GREEN' as const : exceptions <= 9 ? 'AMBER' as const : 'RED' as const;
  ...
  private classifyRiskFactors(items: any[]): Array<{ riskClass: string; es: number; liquidityHorizon: number }> {
    ...
    return Array.from(totalByCategory.entries()).map(([riskClass, balance]) => ({
      riskClass,
  ...
  private getItemRiskClass(item: any): string {
    ...
    if (sub.includes('treasury') || sub.includes('securities')) return 'interest_rate_large';
    if (sub.includes('mortgage') || sub.includes('mbs')) return 'credit_spread_ig';
    if (sub.includes('commercial') || sub.includes('consumer')) return 'credit_spread_hy';
  ...
  private getDemoResult(): FRTBResult {
    return {
      expectedShortfall975: 8.4, liquidityAdjustedES: 12.1, stressedES: 21.0,
      imcc: 14.5, capitalCharge: 25.0, multiplier: 1.5,
      backtestExceptions: 3, backtestTrafficLight: 'GREEN',
      byRiskClass: [
