...
export class SOFRMonitorService {
  ...
  async getExposureReport(institutionId: string): Promise<SOFRMonitorResult> {
    ...
    const exposures: LIBORExposure[] = items
      .filter(i => {
        ...
        return rt === 'variable' && (name.includes('libor') || name.includes('floating'));
      })
      .map(item => {
        const tenor = item.duration > 0.5 ? '3M_LIBOR' : '1M_LIBOR';
        const spread = LIBOR_SOFR_SPREADS[tenor] ?? LIBOR_SOFR_SPREADS['3M_LIBOR'];
    ...
    const sofrExposure = items.filter(i => (i.name || '').toLowerCase().includes('sofr'))
    ...
    return {
      ...
      pctPortfolioExposed: totalPortfolio > 0 ? Math.round((totalLIBOR / totalPortfolio) * 10000) / 100 : 0,
  ...
  private getChecklist() {
    return [
      { item: 'Inventory all LIBOR-referenced instruments', itemEs: 'Inventariar todos los instrumentos referenciados a LIBOR', status: 'complete' as const },
      { item: 'Review fallback language in loan documents', itemEs: 'Revisar cláusulas de respaldo en documentos de préstamo', status: 'in_progress' as const },
      { item: 'Calculate ISDA spread adjustments', itemEs: 'Calcular ajustes de spread ISDA', status: 'complete' as const },
      { item: 'Notify affected borrowers of rate conversion', itemEs: 'Notificar a prestatarios afectados sobre conversión de tasa', status: 'pending' as const },
      { item: 'Update core banking system rate indices', itemEs: 'Actualizar índices de tasas en sistema core bancario', status: 'pending' as const },
      { item: 'File OCIF SOFR transition attestation', itemEs: 'Presentar atestación de transición SOFR a OCIF', status: 'pending' as const },
      { item: 'Board resolution approving transition plan', itemEs: 'Resolución de junta aprobando plan de transición', status: 'in_progress' as const },
    ];
  }
