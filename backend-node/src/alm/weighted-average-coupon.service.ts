...
export class WeightedAverageCouponService {
  calculate(
    ...
    wac: number;
    wam: number;
    waol: number; // weighted avg original life
    totalBalance: number;
    segments: Array<{
      name: string;
      balance: number;
    ...
    const wac =
      loans.reduce((s, l) => s + l.couponRate * l.balance, 0) / totalBalance;
    const wam =
      loans.reduce((s, l) => s + l.remainingTermMonths * l.balance, 0) /
      totalBalance;
    const waol =
      loans.reduce((s, l) => s + l.originalTermMonths * l.balance, 0) /
      totalBalance;
    ...
    const segments = loans.map((l) => ({
      ...
      coupon: l.couponRate,
      remainingMonths: l.remainingTermMonths,
    }));
