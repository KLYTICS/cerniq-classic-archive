...
export class ConcentrationService {
  ...
  async getConcentrationAnalysis(institutionId: string): Promise<ConcentrationAnalysis> {
    const items = await this.prisma.balanceSheetItem.findMany({ where: { institutionId } });
    const customLimits = await this.prisma.concentrationLimit.findMany({ where: { institutionId } });
    ...
    for (const item of assets) {
      ...
      sectorBalances.set(sector, (sectorBalances.get(sector) ?? 0) + item.balance);
    }
    ...
    if (customLimits.length > 0) {
      for (const limit of customLimits) {
        const balance = sectorBalances.get(this.normalizeSectorName(limit.limitName)) ?? 0;
        ...
        exposures.push({
          ...
          headroom,
          status: currentPct > limit.maxPct ? 'breach' : utilization > 0.80 ? 'warning' : 'compliant',
      ...
      for (const [name, config] of Object.entries(DEFAULT_LIMITS)) {
        ...
        if (balance > 0 || config.type === 'single_name') {
          exposures.push({
            ...
            currentBalance: balance,
            headroom,
            status: currentPct > config.maxPct ? 'breach' : utilization > 0.80 ? 'warning' : 'compliant',
            utilizationPct: utilization * 100,
          });
        }
      }
    ...
    // HHI calculation
    const shares = Array.from(sectorBalances.values()).map((b) => (b / totalAssets) * 100);
    const hhi = shares.reduce((sum, s) => sum + s * s, 0);
    const hhiInterpretation = hhi < 1500 ? 'Well diversified' : hhi < 2500 ? 'Moderate concentration' : 'Highly concentrated';
  ...
  async saveConcentrationLimits(institutionId: string, limits: Array<{
    limitType: string;
    limitName: string;
    maxPct: number;
  }>) {
    await this.prisma.concentrationLimit.deleteMany({ where: { institutionId } });
  ...
  private getDemoAnalysis(): ConcentrationAnalysis {
    return {
      exposures: [
        { limitName: 'Commercial RE', limitType: 'sector', maxPct: 0.30, currentPct: 0.27, currentBalance: 120, headroom: 0.03, status: 'warning', utilizationPct: 90 },
        { limitName: 'Residential Mortgage', limitType: 'sector', maxPct: 0.35, currentPct: 0.21, currentBalance: 95, headroom: 0.14, status: 'compliant', utilizationPct: 60 },
        { limitName: 'Consumer Loans', limitType: 'sector', maxPct: 0.25, currentPct: 0.19, currentBalance: 85, headroom: 0.06, status: 'compliant', utilizationPct: 76 },
        { limitName: 'Auto Loans', limitType: 'sector', maxPct: 0.15, currentPct: 0.14, currentBalance: 62, headroom: 0.01, status: 'warning', utilizationPct: 93 },
        { limitName: 'C&I Loans', limitType: 'sector', maxPct: 0.25, currentPct: 0.12, currentBalance: 55, headroom: 0.13, status: 'compliant', utilizationPct: 48 },
        { limitName: 'Credit Cards', limitType: 'product', maxPct: 0.10, currentPct: 0.06, currentBalance: 28, headroom: 0.04, status: 'compliant', utilizationPct: 60 },
      ],
      hhi: 1850,
