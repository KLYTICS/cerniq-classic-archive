...
export class BaselCapitalChargeService {
  calculate(exposures: Array<{ assetClass: string; assetClassEs: string; balance: number; riskWeight: number }>): {
    rwa: number; capitalCharge: number; capitalRatio8Pct: number;
    breakdown: Array<{ assetClass: string; assetClassEs: string; balance: number; riskWeight: number; rwa: number; capitalCharge: number }>;
    interpretation: string; interpretationEs: string;
  } {
    const breakdown = exposures.map(e => ({
      ...e, rwa: +(e.balance * e.riskWeight / 100).toFixed(0),
    ...
    const capitalRatio8Pct = +(capitalCharge / totalExposure * 100).toFixed(2);

    return {
      rwa, capitalCharge, capitalRatio8Pct, breakdown,
