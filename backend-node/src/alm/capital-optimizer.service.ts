...
const ASSET_RATES: Record<string, number> = {
  cash: 0.048, securities: 0.042, consumer_loans: 0.072, auto_loans: 0.065,
  residential_mortgage: 0.055, commercial_re: 0.058, commercial_loans: 0.068,
  credit_cards: 0.145,
};

const LIABILITY_RATES: Record<string, number> = {
  demand_deposits: 0.005, savings: 0.015, time_deposits: 0.040,
  money_market: 0.025, borrowings: 0.052,
};
...
const CONSTRAINTS = {
  ...
  NSFR_MIN: 100,
  NWR_MIN: 7,
  MAX_REALLOCATION_PCT: 0.10, // max 10% of total assets per move
  CONCENTRATION_MAX: 0.30,    // max 30% in any single sector
};
...
export class CapitalOptimizerService {
  ...
  async optimize(
    ...
    // Current NII
    const currentAssetIncome = assets.reduce((s, i) => s + i.balance * i.rate, 0);
    const currentLiabCost = liabilities.reduce((s, i) => s + i.balance * i.rate, 0);
    ...
    const assetsBySubcategory = new Map<string, { balance: number; rate: number; items: any[] }>();
    ...
    for (const [, entry] of assetsBySubcategory) {
      entry.rate = entry.balance > 0
        ? entry.items.reduce((s: number, i: any) => s + i.rate * i.balance, 0) / entry.balance
        : 0;
    }

    // Sort by rate ascending (move FROM low-yield)
    const sorted = Array.from(assetsBySubcategory.entries())
    ...
    if (lowestYield && highestYield && lowestYield[1].rate < highestYield[1].rate) {
    ...
    const liabsBySubcategory = new Map<string, { balance: number; rate: number }>();
    ...
    for (const [sub, entry] of liabsBySubcategory) {
      ...
      entry.rate = entry.balance > 0
        ? items.reduce((s, i) => s + i.rate * i.balance, 0) / entry.balance : 0;
    }
    ...
    const constraintSlacks = [
      { constraint: 'LCR ≥ 100%', currentValue: lcr, limit: CONSTRAINTS.LCR_MIN, slack: lcr - CONSTRAINTS.LCR_MIN, binding: lcr - CONSTRAINTS.LCR_MIN < 5 },
      { constraint: 'NSFR ≥ 100%', currentValue: nsfr, limit: CONSTRAINTS.NSFR_MIN, slack: nsfr - CONSTRAINTS.NSFR_MIN, binding: nsfr - CONSTRAINTS.NSFR_MIN < 5 },
      { constraint: 'NWR ≥ 7%', currentValue: +nwr.toFixed(1), limit: CONSTRAINTS.NWR_MIN, slack: +(nwr - CONSTRAINTS.NWR_MIN).toFixed(1), binding: nwr - CONSTRAINTS.NWR_MIN < 1 },
      { constraint: `Max reallocation ≤ ${(maxMovePct * 100).toFixed(0)}%`, currentValue: deltaAllocations.length > 0 ? +Math.abs(deltaAllocations[0].deltaPct).toFixed(1) : 0, limit: maxMovePct * 100, slack: 0, binding: false },
    ];

    const narrativeEn = totalNIIGain > 0
    ...
    const narrativeEs = totalNIIGain > 0
    ...
    return {
      ...
      projectedNIIGainPct: currentNII > 0 ? Math.round((totalNIIGain / currentNII) * 10000) / 100 : 0,
      constraintSlacks,
  ...
  private getDemoResult(aggressiveness: string): OptimizationResult {
    return {
      deltaAllocations: [
        { subcategory: 'securities', category: 'asset', currentBalance: 50, suggestedBalance: 35, deltaUSD: -15, deltaPct: -3.4, rateImpact: -0.63 },
        { subcategory: 'consumer_loans', category: 'asset', currentBalance: 85, suggestedBalance: 100, deltaUSD: 15, deltaPct: 3.4, rateImpact: 1.08 },
      ],
      ...
      constraintSlacks: [
        ...
        { constraint: 'Max reallocation ≤ 6%', currentValue: 3.4, limit: 6, slack: 2.6, binding: false },
      ],
      aggressivenessLevel: aggressiveness as any,
      narrative: 'Shift $15M from securities to consumer loans to gain $0.45M in annual NII (+3.5%).',
      narrativeEs: 'Traslade $15M de valores a préstamos de consumo para ganar $0.45M en NII anual (+3.5%).',
    };
  }
}
