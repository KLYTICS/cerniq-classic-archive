...
const PR_HURRICANE_AAL: Record<string, { probPerYear: number; lossPct: number }> = {
  ...
  cat5: { probPerYear: 0.005, lossPct: 0.60 },
};

const FEMA_HAIRCUTS: Record<string, number> = {
  AE: 0.25, AO: 0.20, VE: 0.40, X: 0.05, D: 0.15, UNKNOWN: 0.12,
};
...
export interface ClimateRiskResult {
  ...
  scenarios: Array<{ category: string; probability: number; portfolioLoss: number; nwrImpact: number }>;
  narrativeEs: string;
  narrativeEn: string;
}
...
export class ClimateRiskService {
  ...
  async computeClimateRisk(institutionId: string): Promise<ClimateRiskResult> {
    ...
    const totalAssets = items.reduce((s, i) => s + i.balance, 0) || 445;

    const reLoans = items.filter(i =>
      i.category === 'asset' && ['residential_mortgage', 'commercial_re'].includes(i.subcategory)
    );
    const totalREExposure = reLoans.reduce((s, i) => s + i.balance, 0);

    // Hurricane AAL across all categories
    const scenarios = Object.entries(PR_HURRICANE_AAL).map(([cat, { probPerYear, lossPct }]) => {
      const portfolioLoss = totalREExposure * lossPct;
      const nwrImpact = totalAssets > 0 ? (portfolioLoss / totalAssets) * 100 : 0;
      return {
    ...
    const hurricaneAAL = scenarios.reduce((s, sc) => s + sc.probability * sc.portfolioLoss, 0);
    const hurricaneAALPct = totalAssets > 0 ? (hurricaneAAL / totalAssets) * 100 : 0;

    // FEMA flood zone exposure
    const floodZoneExposure = reLoans.reduce((sum, loan) => {
      const zone = (loan as any).floodZone ?? 'UNKNOWN';
      return sum + loan.balance * (FEMA_HAIRCUTS[zone] ?? FEMA_HAIRCUTS.UNKNOWN);
    }, 0);

    const cat3 = scenarios.find(s => s.category.includes('3'))!;
    const cat5 = scenarios.find(s => s.category.includes('5'))!;

    const riskLevel = cat3.nwrImpact > 3 ? 'HIGH' : cat3.nwrImpact > 1.5 ? 'MEDIUM' : 'LOW';

    // Mitigation score based on: insurance coverage, HQLA buffer, geographic diversification
    const hqlaPct = items.filter(i => ['cash', 'securities'].includes(i.subcategory)).reduce((s, i) => s + i.balance, 0) / totalAssets;
    const mitigationScore = Math.min(100, Math.round(hqlaPct * 200 + (totalREExposure < totalAssets * 0.4 ? 30 : 0)));
