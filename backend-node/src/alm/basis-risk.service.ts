...
export class BasisRiskService {
  analyze(params: {
    ...
    const variance =
      params.historicalSpread.reduce((s, v) => s + (v - mean) ** 2, 0) /
      (n - 1);
    const vol = Math.sqrt(variance);
    const zScore = Math.abs((params.currentSpread - mean) / (vol || 1));
    const risk = zScore < 1 ? 'low' : zScore < 2 ? 'moderate' : 'high';
    return {
      meanSpread: +mean.toFixed(4),
      spreadVol: +vol.toFixed(4),
      currentVsHistorical: +zScore.toFixed(2),
      risk,
