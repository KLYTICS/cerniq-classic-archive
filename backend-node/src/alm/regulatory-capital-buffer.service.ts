...
export class RegulatoryCapitalBufferService {
  analyze(params: {
    capitalRatio: number; minimumRatio: number; totalAssets: number;
    quarterlyEarnings: number; stressedLossRate: number;
  }): {
    currentBuffer: number; bufferDollars: number; runwayQuarters: number;
    burnRate: number; breakEvenLoss: number;
    interpretation: string; interpretationEs: string;
  } {
    const { capitalRatio, minimumRatio, totalAssets, quarterlyEarnings, stressedLossRate } = params;
    ...
    const runwayQuarters = netQuarterlyBurn > 0 ? Math.floor(bufferDollars / netQuarterlyBurn) : 99;
    const breakEvenLoss = (quarterlyEarnings / totalAssets) * 400;

    return {
      currentBuffer: +buffer.toFixed(2), bufferDollars: +bufferDollars.toFixed(0),
      runwayQuarters, burnRate: +netQuarterlyBurn.toFixed(0),
