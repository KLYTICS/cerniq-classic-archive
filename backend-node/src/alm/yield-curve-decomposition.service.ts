...
export class YieldCurveDecompositionService {
  decompose(
    prevCurve: Array<{ tenor: number; rate: number }>,
    currCurve: Array<{ tenor: number; rate: number }>,
  ): {
    levelShift: number;
    slopeChange: number;
    ...
    const changes = prevCurve.map((p, i) => {
      ...
      return {
        ...
        prevRate: p.rate,
        currRate: c.rate,
        changeBps: +((c.rate - p.rate) * 10000).toFixed(1),
      };
    });

    const avgChange =
      changes.reduce((s, c) => s + c.changeBps, 0) / changes.length;
    const shortChange =
      changes.filter((c) => c.tenor <= 2).reduce((s, c) => s + c.changeBps, 0) /
      changes.filter((c) => c.tenor <= 2).length;
    const longChange =
      changes
        .filter((c) => c.tenor >= 10)
        .reduce((s, c) => s + c.changeBps, 0) /
      changes.filter((c) => c.tenor >= 10).length;
    const midChange =
      changes
        .filter((c) => c.tenor >= 3 && c.tenor <= 7)
        .reduce((s, c) => s + c.changeBps, 0) /
      Math.max(1, changes.filter((c) => c.tenor >= 3 && c.tenor <= 7).length);

    const levelShift = +avgChange.toFixed(1);
    const slopeChange = +(longChange - shortChange).toFixed(1);
    const curvatureChange = +(
      midChange -
      (shortChange + longChange) / 2
    ).toFixed(1);
    ...
    const dominant =
      absLevel >= absSlope && absLevel >= absCurv
        ? 'level'
        : absSlope >= absCurv
          ? 'slope'
          : 'curvature';
