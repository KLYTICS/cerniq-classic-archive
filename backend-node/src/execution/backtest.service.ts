...
export class BacktestService {
    ...
    async runBacktest(config: BacktestConfig): Promise<BacktestResult> {
        const { strategy, tickers, startDate, endDate, initialCapital, commission } = config;

        this.logger.log(`Starting backtest: ${strategy.name} from ${startDate} to ${endDate}`);
        ...
        for (const dateStr of sortedDates) {
            ...
            const signals = this.generateSignals(strategy, historicalData, dateStr, state);
    ...
    private generateSignals(
        ...
        for (const [ticker, bars] of data) {
            ...
            const lookback = bars.slice(currentIdx - strategy.lookbackPeriod, currentIdx + 1);
            const prices = lookback.map((b) => b.close);

            switch (strategy.type) {
                case 'SMA_CROSSOVER':
                    const smaShort = this.calculateSMA(prices, strategy.params.shortPeriod);
                    const smaLong = this.calculateSMA(prices, strategy.params.longPeriod);
                    const prevShort = this.calculateSMA(prices.slice(0, -1), strategy.params.shortPeriod);
                    const prevLong = this.calculateSMA(prices.slice(0, -1), strategy.params.longPeriod);

                    // Bullish crossover
                    if (prevShort <= prevLong && smaShort > smaLong) {
                        signals.push({ ticker, action: 'BUY', reason: 'SMA bullish crossover' });
                    }
                    // Bearish crossover
                    if (prevShort >= prevLong && smaShort < smaLong) {
                        signals.push({ ticker, action: 'SELL', reason: 'SMA bearish crossover' });
                    }
                    break;
                ...
                case 'RSI_REVERSAL':
                    ...
                    if (rsi < strategy.params.oversold && !position) {
                        signals.push({ ticker, action: 'BUY', reason: `RSI oversold (${rsi.toFixed(1)})` });
                    }
                    // RSI overbought - sell signal
                    if (rsi > strategy.params.overbought && position) {
                        signals.push({ ticker, action: 'SELL', reason: `RSI overbought (${rsi.toFixed(1)})` });
                    }
                    break;
                ...
                case 'MOMENTUM':
                    ...
                    if (momentum > strategy.params.momentumThreshold && !hasPosition) {
                        signals.push({ ticker, action: 'BUY', reason: `Momentum ${momentum.toFixed(1)}%` });
                    }
                    if (momentum < -strategy.params.momentumThreshold && hasPosition) {
                        signals.push({ ticker, action: 'SELL', reason: `Momentum ${momentum.toFixed(1)}%` });
                    }
                    break;
            }
        }
     ...
     */
    private calculatePortfolioValue(state: PortfolioState, prices: Map<string, number>): number {
     ...
     * Calculate performance metrics
     */
    private calculateMetrics(state: PortfolioState, initialCapital: number): BacktestMetrics {
        ...
        const winningTrades = state.trades.filter((t, i, arr) => {
            ...
            const buyTrade = arr.slice(0, i).reverse().find(
                (bt) => bt.ticker === t.ticker && bt.action === 'BUY',
            );
