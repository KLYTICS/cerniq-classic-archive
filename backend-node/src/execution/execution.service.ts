...
export class ExecutionService {
     ...
     */
    async analyzeVWAP(execution: ExecutionDto, periodMinutes: number = 60): Promise<VWAPAnalysis> {
        const { ticker, executionPrice, executionTime, side, quantity } = execution;
    ...
    async generateBestExecutionReport(
        ...
        const avgSlippageBps = analyses.length > 0
        ...
        return {
            ...
            summary: {
                ...
                qualityScore: Number((
                    (excellentCount * 4 + goodCount * 3 + fairCount * 2 + poorCount * 1) /
                    (executions.length * 4) * 100
                ).toFixed(1)),
            },
            executions: analyses,
    ...
    private checkComplianceFlags(analyses: SlippageAnalysis[]): string[] {
        ...
        if (highSlippage.length > 0) {
            flags.push(`HIGH_SLIPPAGE_DETECTED: ${highSlippage.length} executions >50bps slippage`);
        }

        const largeOrders = analyses.filter((a) => a.notional > 1000000);
        for (const order of largeOrders) {
            if (order.quality === 'POOR') {
                flags.push(`LARGE_ORDER_POOR_FILL: ${order.ticker} $${order.notional.toFixed(0)} rated POOR`);
            }
        }
    ...
    async calculateImplementationShortfall(
        ...
        const delayComponent = (benchmarkPrice - decisionPrice) / decisionPrice * 10000;
        const tradingComponent = (avgExecutionPrice - benchmarkPrice) / benchmarkPrice * 10000;
        const totalShortfall = delayComponent + tradingComponent;

        const shortfallDollars = (avgExecutionPrice - decisionPrice) * totalQuantity;
