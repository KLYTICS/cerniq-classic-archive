...
describe('ExpensesController', () => {
  ...
  it('findOne delegates to ExpensesService', async () => {
    const req = { headers: { 'x-organization-id': 'org-1' }, user: { userId: 'u1' } };
    ...
    expect(mockExpensesService.findOne).toHaveBeenCalledWith('e1', 'org-1', 'u1');
  });
  ...
  it('update delegates to ExpensesService', async () => {
    ...
    expect(mockExpensesService.update).toHaveBeenCalledWith('e1', 'org-1', 'u1', dto);
  });

  // ── submit ───────────────────────────────────────────────────────────
  it('submit delegates to ExpensesService', async () => {
    const req = { headers: { 'x-organization-id': 'org-1' }, user: { userId: 'u1' } };
    mockExpensesService.submit.mockResolvedValue({ id: 'e1', status: 'SUBMITTED' });

    const result = await controller.submit('e1', req);

    expect(result).toEqual({ id: 'e1', status: 'SUBMITTED' });
    expect(mockExpensesService.submit).toHaveBeenCalledWith('e1', 'org-1', 'u1');
  });

  // ── approve ──────────────────────────────────────────────────────────
  it('approve delegates to ExpensesService', async () => {
    const req = { headers: { 'x-organization-id': 'org-1' }, user: { userId: 'u1' } };
    mockExpensesService.approve.mockResolvedValue({ id: 'e1', status: 'APPROVED' });

    const result = await controller.approve('e1', req);

    expect(result).toEqual({ id: 'e1', status: 'APPROVED' });
    expect(mockExpensesService.approve).toHaveBeenCalledWith('e1', 'org-1', 'u1');
  });

  // ── reject ───────────────────────────────────────────────────────────
  it('reject delegates to ExpensesService', async () => {
    const req = { headers: { 'x-organization-id': 'org-1' }, user: { userId: 'u1' } };
    mockExpensesService.reject.mockResolvedValue({ id: 'e1', status: 'REJECTED' });

    const result = await controller.reject('e1', req);

    expect(result).toEqual({ id: 'e1', status: 'REJECTED' });
    expect(mockExpensesService.reject).toHaveBeenCalledWith('e1', 'org-1', 'u1');
  });
  ...
  it('remove delegates to ExpensesService', async () => {
    ...
    expect(mockExpensesService.remove).toHaveBeenCalledWith('e1', 'org-1', 'u1');
  });
  ...
  it('processReceipt delegates to ExpensesService with resolved org', async () => {
    ...
    mockExpensesService.processReceipt.mockResolvedValue({ id: 'e1', aiExtracted: true });

    const result = await controller.processReceipt(dto, req);

    expect(result).toEqual({ id: 'e1', aiExtracted: true });
    expect(mockExpensesService.processReceipt).toHaveBeenCalledWith('org-1', 'u1', dto);
  });

  // ── analyzeOrganization ──────────────────────────────────────────────
  it('analyzeOrganization verifies membership then delegates', async () => {
    const req = { user: { userId: 'u1' } };
    mockPrisma.organizationMember.findUnique.mockResolvedValue({ userId: 'u1', organizationId: 'org-1' });
    mockAnomalyDetection.analyzeOrganization.mockResolvedValue({ anomalies: [] });
  ...
  it('analyzeOrganization throws when user is not a member', async () => {
    ...
    await expect(controller.analyzeOrganization('org-1', req)).rejects.toThrow(BadRequestException);
  });

  // ── getVendorReport ──────────────────────────────────────────────────
  it('getVendorReport verifies membership, queries expenses, delegates', async () => {
    const req = { user: { userId: 'u1' } };
    mockPrisma.organizationMember.findUnique.mockResolvedValue({ userId: 'u1' });
    mockPrisma.expense.findMany.mockResolvedValue([
      { merchantName: 'Vendor A', amount: 100, transactionDate: new Date('2026-01-01') },
    ]);
    mockVendorIntel.generateVendorReport.mockResolvedValue({ vendors: [], concentration: 0 });
  ...
  it('getLiquidityImpact verifies membership then delegates', async () => {
    const req = { user: { userId: 'u1' } };
    mockPrisma.organizationMember.findUnique.mockResolvedValue({ userId: 'u1' });
    mockAnomalyDetection.calculateApLcrImpact.mockResolvedValue({ impact: 0.05 });

    const result = await controller.getLiquidityImpact('org-1', 'inst-1', req);

    expect(result).toEqual({ impact: 0.05 });
    expect(mockAnomalyDetection.calculateApLcrImpact).toHaveBeenCalledWith('org-1', 'inst-1');
  });
  ...
  it('getTemplate returns inline CSV template when file does not exist', async () => {
    ...
    expect(res.send).toHaveBeenCalledWith(expect.stringContaining('date,invoice_number,vendor'));
  });
  ...
  it('uploadExpenseCSV throws when no file provided', async () => {
    ...
    await expect(controller.uploadExpenseCSV(req, 'org-1', undefined as any)).rejects.toThrow(BadRequestException);
  });

  it('uploadExpenseCSV returns errors when CSV is invalid', async () => {
    const req = { user: { userId: 'u1' } };
    const file = { buffer: Buffer.from('bad'), originalname: 'test.csv', size: 3 } as Express.Multer.File;
    mockIngestion.parseExpenseCSV.mockReturnValue({ valid: false, errors: ['bad header'], warnings: [], summary: null, items: [] });
  ...
  it('uploadExpenseCSV with auto orgId resolves from membership', async () => {
    const req = { user: { userId: 'u1' } };
    const file = { buffer: Buffer.from('good'), originalname: 'test.csv', size: 4 } as Express.Multer.File;
    mockIngestion.parseExpenseCSV.mockReturnValue({
      valid: true, errors: [], warnings: [], summary: {}, items: [
        { vendor: 'V1', amount: 100, currency: 'USD', category: 'supplies', date: '2026-01-01', status: 'PAID' },
      ],
    });
    mockPrisma.organizationMember.findFirst.mockResolvedValue({ organizationId: 'auto-org' });
  ...
  it('uploadExpenseCSV creates default org when user has no membership', async () => {
    ...
    mockIngestion.parseExpenseCSV.mockReturnValue({
      valid: true, errors: [], warnings: [], summary: {}, items: [
        { vendor: 'V1', amount: 50, currency: 'USD', category: 'utilities', date: '2026-01-01', status: 'PENDING' },
      ],
    });
  ...
  it('uploadExpenseCSV verifies membership for specific orgId', async () => {
    ...
    await expect(controller.uploadExpenseCSV(req, 'specific-org', file)).rejects.toThrow(BadRequestException);
  });
  ...
  it('uploadExpenseCSV handles anomaly detection failure gracefully', async () => {
    ...
    mockPrisma.organizationMember.findFirst.mockResolvedValue({ organizationId: 'org-1' });
    mockPrisma.expense.create.mockResolvedValue({ id: 'exp-1' });
    mockAnomalyDetection.analyzeOrganization.mockRejectedValue(new Error('Analysis failed'));
  ...
  it('resolveOrgId falls back to user.orgId when header is missing', async () => {
    ...
    expect(mockExpensesService.findAll).toHaveBeenCalledWith('user-org', 'u1', undefined);
  });
  ...
  it('findAll passes status filter to service', async () => {
    ...
    expect(mockExpensesService.findAll).toHaveBeenCalledWith('org-1', 'u1', 'APPROVED');
  });
  ...
  it('generateAPReport verifies membership and returns PDF buffer', async () => {
    ...
    expect(mockApReport.generateAPReport).toHaveBeenCalledWith('org-1', 'inst-1', 'es');
  ...
  it('generateAPReport defaults to English when lang is not es', async () => {
    ...
    expect(mockApReport.generateAPReport).toHaveBeenCalledWith('org-1', null, 'en');
  });
});
