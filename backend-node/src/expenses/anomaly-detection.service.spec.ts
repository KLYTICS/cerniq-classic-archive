...
describe('AnomalyDetectionService', () => {
  ...
  it('detects near-duplicate invoices (same vendor, close amount, close date)', async () => {
    ...
    const result = await service.analyzeOrganization('org-1');
    const nearDupes = result.findings.filter(
      (f) => f.findingType === 'DUPLICATE_INVOICE' && f.severity === 'MEDIUM',
    );
  ...
  it('detects amount anomalies (outlier)', async () => {
    ...
    mockPrisma.expense.findMany.mockResolvedValue(expenses);
    const result = await service.analyzeOrganization('org-1');
    const amountAnoms = result.findings.filter(
  ...
  it('detects split billing (round thousand in 14-day window)', async () => {
    ...
    mockPrisma.expense.findMany.mockResolvedValue([
      {
        id: 'e1',
        merchantName: 'SplitCo',
        amount: 333,
        category: null,
        description: null,
        transactionDate: new Date(base.getTime()),
        reviewStatus: 'PENDING',
        createdAt: base,
      },
      {
        ...
        category: null,
        description: null,
        transactionDate: new Date(base.getTime() + 2 * 86400000),
        reviewStatus: 'PENDING',
        createdAt: base,
      },
      {
        ...
        category: null,
        description: null,
        transactionDate: new Date(base.getTime() + 5 * 86400000),
        reviewStatus: 'PENDING',
        createdAt: base,
      },
    ]);
    const result = await service.analyzeOrganization('org-1');
    const splits = result.findings.filter(
      (f) => f.findingType === 'SPLIT_BILLING',
    );
  ...
  it('detects frequency anomalies (spike in latest month)', async () => {
    ...
    for (let m = 1; m <= 3; m++) {
      for (let i = 0; i < 3; i++) {
        expenses.push({
          id: `e-m${m}-${i}`,
          merchantName: 'FreqVendor',
          amount: 500,
          category: null,
          description: null,
          transactionDate: new Date(
            `2026-0${m}-${String(i + 10).padStart(2, '0')}`,
          ),
          reviewStatus: 'PENDING',
          createdAt: new Date(),
        });
    ...
    mockPrisma.expense.findMany.mockResolvedValue(expenses);
    const result = await service.analyzeOrganization('org-1');
    const freqAnoms = result.findings.filter(
      (f) => f.findingType === 'FREQUENCY_ANOMALY',
    );
  ...
  it('detects dormant vendor reactivation (gap > 90 days)', async () => {
    ...
    const result = await service.analyzeOrganization('org-1');
    const dormant = result.findings.filter(
      (f) => f.findingType === 'DORMANT_VENDOR_REACTIVATED',
    );
  ...
  it('detects unauthorized category (IT with catering keyword)', async () => {
    ...
    const result = await service.analyzeOrganization('org-1');
    const catAnoms = result.findings.filter(
      (f) => f.findingType === 'UNAUTHORIZED_CATEGORY',
    );
  ...
  it('detects medium vendor concentration (25-35%)', async () => {
    ...
    mockPrisma.expense.findMany.mockResolvedValue(expenses);
    const result = await service.analyzeOrganization('org-1');
    const medConc = result.findings.filter(
      (f) =>
        f.findingType === 'VENDOR_CONCENTRATION' && f.severity === 'MEDIUM',
    );
  ...
  it('calculateApLcrImpact returns DATA_UNAVAILABLE when LCR lookup fails', async () => {
    ...
    mockAlmEnterprise.calculateLCR.mockRejectedValue(
      new Error('LCR unavailable'),
    );
    const result = await service.calculateApLcrImpact('org-1', 'inst-1');
  ...
  it('detects HIGH severity frequency anomaly (> 4x rolling avg)', async () => {
    ...
    mockPrisma.expense.findMany.mockResolvedValue(expenses);
    const result = await service.analyzeOrganization('org-1');
    const hiFreq = result.findings.filter(
      (f) => f.findingType === 'FREQUENCY_ANOMALY' && f.severity === 'HIGH',
    );
  ...
  it('detects HIGH severity dormant vendor (> 180 day gap)', async () => {
    ...
    const dormant = result.findings.filter(
      (f) =>
        f.findingType === 'DORMANT_VENDOR_REACTIVATED' && f.severity === 'HIGH',
    );
    expect(dormant.length).toBeGreaterThan(0);
  });
