...
import { ReceiptParserService, ParsedReceipt } from '../llm/receipt-parser.service';
...
export class ExpensesService {
    ...
    async processReceipt(
        ...
        const parsed: ParsedReceipt = await this.receiptParser.parseReceipt(dto.receiptUrl);
        ...
        const existingParsed = recentExpenses
            .filter((e) => e.aiData)
            .map((e) => e.aiData as any as ParsedReceipt);

        const isDuplicate = await this.receiptParser.checkDuplicate(parsed, existingParsed);
     ...
     */
    async findAll(organizationId: string, userId: string, status?: ExpenseStatus) {
    ...
    private async isAdmin(organizationId: string, userId: string): Promise<boolean> {
        const member = await this.prisma.organizationMember.findUnique({
            where: {
                organizationId_userId: {
                    organizationId,
