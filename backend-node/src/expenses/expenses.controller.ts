...
export class ExpensesController {
    ...
    async uploadExpenseCSV(
        ...
        this.logger.log(`SpendCheck CSV upload for org ${orgId} (${file.size} bytes, file: ${file.originalname})`);
                ...
                batch.map((item) =>
                    this.prisma.expense.create({
                        data: {
                            ...
                            status: item.status === 'PAID' ? 'APPROVED'
                                : item.status === 'PENDING' ? 'SUBMITTED'
                                : 'DRAFT',
                            aiExtracted: false,
                        },
                        select: { id: true },
        ...
        this.logger.log(`SpendCheck CSV: ingested ${created.length} expenses for org ${resolvedOrgId}`);
        ...
        try {
            ...
            this.logger.log(`SpendCheck anomaly detection completed for org ${resolvedOrgId}`);
        } catch (err) {
            this.logger.error(`SpendCheck anomaly detection failed for org ${resolvedOrgId}`, err);
        }
    ...
    async getTemplate(@Res() res: any) {
        ...
        const templatePath = path.join(__dirname, '..', '..', '..', 'frontend', 'public', 'templates', 'cerniq-spendcheck-template.csv');
        ...
        if (fs.existsSync(templatePath)) {
            ...
            res.set({
                ...
                'Content-Disposition': 'attachment; filename="cerniq-spendcheck-template.csv"',
            });
            res.send(content);
            return;
        }
