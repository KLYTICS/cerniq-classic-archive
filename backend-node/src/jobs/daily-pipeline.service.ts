...
export class DailyPipelineService {
     ...
     */
    @Cron('0 30 23 * * 1-5', { name: 'daily-eod-pipeline', timeZone: 'America/New_York' })
    ...
    async runPipeline(): Promise<{
        ...
        try {
            ...
            this.logger.log(`Active universe: ${tickers.length} tickers — ${tickers.join(', ')}`);

            if (tickers.length === 0) {
                this.logger.warn('No active tickers found in positions table');
                await this.completePipelineRun(run.id, 'SUCCESS', 0, [], startTime);
                return { status: 'SUCCESS', tickersProcessed: 0, errors: [], durationMs: Date.now() - startTime };
            }
            ...
            await this.completePipelineRun(run.id, status, tickersProcessed, errors, startTime);
            ...
            await this.completePipelineRun(run.id, 'FAILED', tickersProcessed, errors, startTime);
    ...
    private async fetchEodPricesInBatches(
        tickers: string[],
        errors: string[],
    ): Promise<{ ticker: string; price: number; change: number; changePercent: number; high?: number; low?: number; open?: number; volume?: number }[]> {
        const results: { ticker: string; price: number; change: number; changePercent: number; high?: number; low?: number; open?: number; volume?: number }[] = [];

        // Process in chunks of BATCH_SIZE
        for (let i = 0; i < tickers.length; i += BATCH_SIZE) {
            const batch = tickers.slice(i, i + BATCH_SIZE);
            this.logger.debug(`Fetching batch ${Math.floor(i / BATCH_SIZE) + 1}: ${batch.join(', ')}`);
    ...
    private async upsertPrices(
        prices: { ticker: string; price: number; high?: number; low?: number; open?: number; volume?: number }[],
        errors: string[],
    ): Promise<void> {
        ...
        this.logger.log(`Upserted ${prices.length} price records for ${today.toISOString().slice(0, 10)}`);
    }
    ...
    private async recomputePortfolioRisk(errors: string[]): Promise<void> {
        ...
        for (const portfolio of portfolios) {
            ...
            try {
                ...
                const correlationResult = await this.riskService.calculateCorrelationMatrix({
                    tickers,
                ...
                const msg = err instanceof Error ? err.message : String(err);
                this.logger.error(`Failed to compute risk for portfolio ${portfolio.id}: ${msg}`);
