...
export class DataQualityService {
    ...
    validatePriceData(data: any[]): DataQualityReport {
        ...
        for (const point of data) {
            ...
            if (data.indexOf(point) > 0) {
                ...
                const changePercent = Math.abs((point.close - prevPoint.close) / prevPoint.close);
    ...
    getHealthStatus(): HealthStatus {
        ...
        const totalSuccess = metrics.reduce((sum, m) => sum + m.successfulRequests, 0);
        ...
        return {
            ...
            details: {
                ...
                avgLatency: Number(
                    (metrics.reduce((sum, m) => sum + m.avgLatencyMs, 0) / metrics.length).toFixed(2),
                ),
            },
        };
    }
