...
describe('AdvancedRiskService', () => {
    ...
    describe('calculateComponentVaR', () => {
        ...
        it('should return portfolio VaR and component breakdown', async () => {
            const result = await service.calculateComponentVaR(testPositions, 0.95, 1);
        ...
        it('should scale VaR with horizon', async () => {
            const result1Day = await service.calculateComponentVaR(testPositions, 0.95, 1);
            const result10Day = await service.calculateComponentVaR(testPositions, 0.95, 10);
    ...
    describe('calculateParametricVaR', () => {
        ...
        it('should return VaR with covariance-based calculation', async () => {
            const result = await service.calculateParametricVaR(testPositions, 0.95, 1);
        ...
        it('should use correct z-score for confidence level', async () => {
            const result95 = await service.calculateParametricVaR(testPositions, 0.95, 1);
            const result99 = await service.calculateParametricVaR(testPositions, 0.99, 1);
...
describe('VaR Backtesting Validation', () => {
    ...
    it('should pass Kupiec test (conceptual validation)', () => {
        ...
        const simulatedViolations = Math.round(observations * expectedViolationRate);

        // Kupiec test statistic
        const actualViolationRate = simulatedViolations / observations;
        const LR =
            -2 *
            (Math.log(Math.pow(1 - actualViolationRate, observations - simulatedViolations) *
                Math.pow(actualViolationRate, simulatedViolations)) -
                Math.log(Math.pow(1 - expectedViolationRate, observations - simulatedViolations) *
